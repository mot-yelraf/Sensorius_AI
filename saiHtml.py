...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, expected_display_style_map=None, display_style=None, astro_payload=None, biodynamic_payload=None, weather_forecast_provider="met_no"):
    ...
    yield "  if (summaryEl) {"
    yield "    const summaryText = dateIso ? bioSummaryWithForecast(dateIso, summaries[dateIso] || '') : '';"
    ...
    yield "}"
    yield "async function fetchBiodynamicMonth(monthKey){"
    yield "  const key = String(monthKey || bioMonthKeyFromDate(new Date()));"
    yield "  window.__bioMonthCache = window.__bioMonthCache || {};"
    yield "  window.__bioMonthInflight = window.__bioMonthInflight || {};"
    yield "  if (window.__bioMonthCache[key]) return window.__bioMonthCache[key];"
    yield "  if (window.__bioMonthInflight[key]) return window.__bioMonthInflight[key];"
