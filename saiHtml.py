...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, display_style=None):
    ...
    yield ".bio-hint{margin-top:auto;padding-top:.35rem;font-size:.54rem;line-height:1.1;text-align:center;color:#6b7280;text-transform:uppercase;letter-spacing:.03em;}"
    yield ".bio-card-button{cursor:pointer;}"
    ...
    yield ".bio-day.today{box-shadow:inset 0 0 0 2px #27313a;}"
    ...
    yield "<div class='astro-box bio-card-button' id='bioBox' aria-live='polite' role='button' tabindex='0' title='Open biodynamic calendar'>"
    ...
    yield "    <div class='bio-hint'>Click to see calendar</div>"
    yield "  </div>"
    yield "</div>"
    ...
    yield "  if (!st.selectedDate && days.length) { const firstInMonth = days.find((d) => d && d.in_month) || days[0]; st.selectedDate = firstInMonth ? firstInMonth.date : ''; }"
    ...
    yield "    });"
    yield "    await loadBiodynamicMonth(monthKey, (window.__bioModalState && window.__bioModalState.selectedDate) || '');"
    yield "  } catch (e) {"
    ...
    yield "  const bioBox = document.getElementById('bioBox');"
    yield "  const btn = document.getElementById('saveBtn');"
    yield "  const spinner = document.getElementById('saveSpinner');"
    yield "  if (bioBox) {"
    yield "    bioBox.addEventListener('click', function(){ if (window.openBiodynamicCalendarModal) window.openBiodynamicCalendarModal(); });"
    yield "    bioBox.addEventListener('keydown', function(ev){ if (ev.key === 'Enter' || ev.key === ' ') { ev.preventDefault(); if (window.openBiodynamicCalendarModal) window.openBiodynamicCalendarModal(); } });"
    yield "  }"
