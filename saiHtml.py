...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, display_style=None):
    ...
    yield "<style>"
    yield ".dash-top-row{display:grid;grid-template-columns:230px max-content 230px;justify-content:center;align-items:start;column-gap:.75rem;row-gap:.75rem;margin:1rem auto 0;max-width:fit-content;}"
    yield ".dash-left-col{display:flex;flex-direction:column;align-items:stretch;gap:.75rem;}"
    yield ".dash-right-col{display:flex;align-items:flex-start;justify-content:flex-start;}"
    yield ".dash-side-col{display:flex;align-items:flex-start;justify-content:flex-start;}"
    yield ".dash-loc-form{display:flex;flex-direction:column;align-items:stretch;justify-content:flex-start;gap:.45rem;background:#e6faff;border:1px solid #c9ddff;border-radius:10px;padding:.45rem .65rem .55rem;min-height:102px;min-width:230px;width:230px;}"
    ...
    yield "#bioBox .astro-card{width:230px;min-width:230px;align-items:stretch;}"
    ...
    yield ".bio-status{display:flex;align-items:center;justify-content:space-between;gap:.45rem;padding:.35rem .45rem;border-radius:8px;border:1px solid #d6ceb4;background:#fffdf6;}"
    yield ".bio-main{display:flex;flex-direction:column;align-items:flex-start;gap:.1rem;min-width:0;}"
    yield "#bioCurrentSign{font-size:.86rem;font-weight:700;line-height:1.1;color:#27313a;}"
    yield "#bioCurrentElement{font-size:.68rem;color:#49545d;line-height:1.15;}"
    yield "#bioCurrentBadge{display:inline-flex;align-items:center;justify-content:center;min-width:60px;padding:.3rem .4rem;border-radius:999px;border:1px solid transparent;font-size:.62rem;font-weight:700;letter-spacing:.03em;text-transform:uppercase;color:#fff;}"
    yield ".bio-window{font-size:.66rem;line-height:1.2;text-align:center;color:#3c464d;min-height:1.7em;padding-top:.2rem;}"
    yield "#bioUpcoming{font-size:.63rem;line-height:1.15;text-align:center;color:#3c464d;min-height:1.5em;}"
    yield ".bio-month{font-size:.68rem;font-weight:700;text-align:center;color:#27313a;padding-top:.15rem;}"
    yield ".bio-calendar{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:3px;margin-top:.2rem;}"
    ...
    yield "    <div class='bio-month' id='bioMonthLabel'>--</div>"
    yield "    <div class='bio-calendar' id='bioCalendar'></div>"
    yield "  </div>"
    yield "</div>"
    yield "</div>"
    ...
    yield "  const monthEl = document.getElementById('bioMonthLabel');"
    yield "  const calEl = document.getElementById('bioCalendar');"
    yield "  if (!signEl || !elementEl || !badgeEl || !panelEl || !windowEl || !upcomingEl || !monthEl || !calEl) return;"
    yield "  const esc = (s) => String(s == null ? '' : s).replace(/[&<>\"']/g, (ch) => ({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',\"'\":'&#39;'}[ch] || ch));"
    ...
    yield "    monthEl.textContent = data && data.month_label ? data.month_label : '';"
    yield "    calEl.innerHTML = '';"
    yield "    return;"
    yield "  }"
    ...
    yield "  monthEl.textContent = data.month_label || '';"
    ...
    yield "  let html = weekdays.map((label) => `<div class='bio-weekday'>${esc(label)}</div>`).join('');"
    ...
    yield "    const style = `background:${esc(day.dominant_accent || '#fff')};border-color:${esc(day.dominant_color || '#d7d0bf')};`;"
    yield "    const title = `${esc(day.date || '')} ${esc(day.dominant_sign || '')} ${esc(day.dominant_plant_part || '')}`.trim();"
    yield "    return `<div class='${classes.join(' ')}' style='${style}' title='${title}'><span class='bio-day-num'>${esc(day.day)}</span></div>`;"
    yield "  }).join('');"
