...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, expected_display_style_map=None, display_style=None, astro_payload=None, biodynamic_payload=None):
    ...
    yield ".bio-status{display:flex;align-items:center;justify-content:flex-start;gap:.14rem;padding:.24rem .3rem;border-radius:8px;border:1px solid #27313a;background:#fffdf6;overflow:hidden;box-sizing:border-box;}"
    ...
    yield "#bioCurrentBadge{display:inline-flex;align-items:center;justify-content:center;min-width:48px;max-width:48px;padding:.22rem .24rem;border-radius:999px;border:1px solid #27313a;font-size:.52rem;font-weight:700;letter-spacing:.01em;text-transform:uppercase;color:#27313a;white-space:nowrap;flex:0 0 auto;overflow:hidden;margin-left:auto;}"
    ...
    yield "    return luminance >= 0.55 ? '#27313a' : '#fff';"
    yield "  };"
    yield "  const biodynamicMainTextColor = (item) => {"
    ...
    yield "    if ((element === 'fire' && part === 'fruit') || (element === 'water' && part === 'leaf')) return '#fff';"
    yield "    if ((element === 'earth' && part === 'root') || (element === 'air' && part === 'flower')) return '#27313a';"
    yield "    return textOnHex(String((item && item.accent) || (item && item.color) || '#7f8a93'));"
    yield "  };"
    ...
    yield "    badgeEl.style.borderColor = '#27313a';"
    yield "    badgeEl.style.color = textOnHex('#7f8a93');"
    yield "    boxEl.style.background = '#ffffe0';"
    yield "    panelEl.style.background = '#f6f7f8';"
    yield "    [titleEl, signEl, elementEl, dateEl, windowEl, upcomingEl].forEach((el) => { if (el) el.style.color = ''; });"
    ...
    yield "  badgeEl.style.borderColor = '#27313a';"
    yield "  badgeEl.style.color = textOnHex(color);"
    yield "  boxEl.style.background = buildRollingGradient(data, cur.timestamp);"
    yield "  panelEl.style.background = accent;"
    yield "  panelEl.style.borderColor = '#27313a';"
    yield "  const mainTextColor = biodynamicMainTextColor(cur);"
    yield "  [titleEl, signEl, elementEl, dateEl, windowEl, upcomingEl].forEach((el) => { if (el) el.style.color = mainTextColor; });"
