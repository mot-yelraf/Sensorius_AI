...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, expected_display_style_map=None, display_style=None, astro_payload=None, biodynamic_payload=None, weather_forecast_provider="met_no"):
    ...
    yield "    if (!Number.isFinite(mr) || !Number.isFinite(ms) || mr >= ms) return moonPoints;"
    yield "    const visible = moonPoints.filter((p) => p.m >= mr && p.m <= ms && Number.isFinite(p.e));"
    yield "    let peak = null;"
    yield "    for (const p of visible) {"
    yield "      if (!peak || p.e > peak.e) peak = p;"
    yield "    }"
    yield "    const peakMin = peak ? Math.max(mr + 1, Math.min(ms - 1, peak.m)) : ((mr + ms) / 2);"
    yield "    const peakYRaw = peak ? yForElev(peak.e) : (yBase - (abovePx * 0.95));"
    yield "    const topY = Math.max(padY + 2, Math.min(yBase - 6, peakYRaw));"
    yield "    const bottomY = yBase + belowPx;"
    yield "    const prevMoonSet = ms - 1440;"
    yield "    const nextMoonRise = mr + 1440;"
    yield "    const prevTrough = prevMoonSet + ((mr - prevMoonSet) / 2);"
    yield "    const nextTrough = ms + ((nextMoonRise - ms) / 2);"
    yield "    const riseSlope = -Math.min(((yBase - topY) * Math.PI) / (2 * Math.max(1, peakMin - mr)), ((bottomY - yBase) * Math.PI) / (2 * Math.max(1, mr - prevTrough)));"
    yield "    const setSlope = Math.min(((yBase - topY) * Math.PI) / (2 * Math.max(1, ms - peakMin)), ((bottomY - yBase) * Math.PI) / (2 * Math.max(1, nextTrough - ms)));"
    yield "    const keys = ["
    yield "      {m: prevMoonSet, y: yBase, s: setSlope},"
    yield "      {m: prevTrough, y: bottomY, s: 0},"
    yield "      {m: mr, y: yBase, s: riseSlope},"
    yield "      {m: peakMin, y: topY, s: 0},"
    yield "      {m: ms, y: yBase, s: setSlope},"
    yield "      {m: nextTrough, y: bottomY, s: 0},"
