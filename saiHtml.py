...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, expected_display_style_map=None, display_style=None, astro_payload=None, biodynamic_payload=None, weather_forecast_provider="met_no"):
    ...
    yield ".switch-dialog-content,.modal .split > section.panel{position:relative}"
    ...
    yield "  window.getToastContainer = function(){"
    ...
    yield "      const selectors = ['.system-settings-content', '.switch-dialog-content', '.modal-body .split > section.panel'];"
    ...
    yield "      const backdrops = Array.from(document.querySelectorAll('.modal-backdrop')).filter(visible);"
    yield "      for (let i = backdrops.length - 1; i >= 0; i -= 1) {"
    yield "        const pane = paneFrom(backdrops[i]);"
    yield "        if (pane) return pane;"
    yield "      }"
    yield "      const loose = Array.from(document.querySelectorAll('.system-settings-shell, .modal')).filter((el) => visible(el) && !el.closest('.modal-backdrop'));"
    yield "      for (let i = loose.length - 1; i >= 0; i -= 1) {"
    yield "        const pane = paneFrom(loose[i]);"
    yield "        if (pane) return pane;"
    yield "      }"
    yield "      return null;"
    yield "    }"
    ...
    yield "    const c = window.getToastContainer ? window.getToastContainer() : null;"
    yield "    if (!c) return;"
...
def core_helpers_html() -> str:
    ...
    return (
        ...
        ".switch-dialog-content,.modal .split > section.panel{position:relative}"
        ...
        "    window.getToastContainer = function(){"
        "      function visible(el){"
        ...
        "        var selectors = ['.system-settings-content', '.switch-dialog-content', '.modal-body .split > section.panel'];"
        ...
        "      }"
        "      function activePane(){"
        "        var backdrops = Array.prototype.filter.call(document.querySelectorAll('.modal-backdrop'), visible);"
        "        for (var i = backdrops.length - 1; i >= 0; i -= 1) {"
        "          var pane = paneFrom(backdrops[i]);"
        "          if (pane) return pane;"
        "        }"
        "        var loose = Array.prototype.filter.call(document.querySelectorAll('.system-settings-shell, .modal'), function(el){ return visible(el) && !el.closest('.modal-backdrop'); });"
        "        for (var j = loose.length - 1; j >= 0; j -= 1) {"
        "          var loosePane = paneFrom(loose[j]);"
        "          if (loosePane) return loosePane;"
        "        }"
        "        return null;"
        "      }"
        "      var pane = activePane();"
        ...
        "      var c = window.getToastContainer ? window.getToastContainer() : null;"
        "      if (!c) return;"
