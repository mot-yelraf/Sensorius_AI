...
def test_sun_position_card_renders_29_day_overlay():
    ...
    assert "const riseMin = mr;" in html
    assert "const setMin = ms > riseMin ? ms : ms + 1440;" in html
    assert "if (!Number.isFinite(mr) || !Number.isFinite(ms) || mr === ms) return [];" in html
    ...
    assert "const amplitude = Math.min(abovePx * 0.94, belowPx * 0.9);" in html
    assert "return yBase - (amplitude * wave);" in html
    assert "Math.tanh(blendStrength * wave)" not in html
