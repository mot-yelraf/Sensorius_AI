...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, expected_display_style_map=None, display_style=None, astro_payload=None, biodynamic_payload=None, weather_forecast_provider="met_no"):
    ...
    yield "  if (metric !== 'wind direction') return;"
    ...
    yield "  if (normalized === 'Graph6hr') {"
    yield "    title.textContent = 'Wind-Rose (6hr)';"
    yield "  } else if (normalized === 'Graph24hr') {"
    yield "    title.textContent = 'Wind-Rose (24hr)';"
    yield "  } else {"
    yield "    const unit = String(gaugeConfig?.['Wind Direction']?.unit || 'deg');"
    yield "    title.textContent = `Wind Direction (${unit})`;"
    yield "  }"
    yield "};"
    ...
    yield "  <img src='/ui_static/01-sensorius-overview-v4.png' alt='Sensorius system overview' loading='lazy'>"
    yield "</div>"
