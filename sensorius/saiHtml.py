...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, expected_display_style_map=None, display_style=None, astro_payload=None, biodynamic_payload=None, weather_forecast_provider="met_no", weather_forecast_theme="pollinator", dashboard_background_theme="leaf", dashboard_metric_set="Pick 6", dashboard_custom_theme_style=""):
    ...
    yield "<a href='javascript:void(0)' onclick='openGraphModal()' title='View Graph' aria-label='Full Screen Graphs' class='dashboard-header-action'>"
    ...
    yield "<p id='dashboard_refresh_status' role='status' hidden>Live updates delayed. Displayed readings may be out of date. Retrying automatically.</p>"
    ...
    yield ".forecast-current-summary{font-size:.68rem;line-height:1.16;color:var(--forecast-ink);text-align:center;height:2.35em;overflow:auto;overflow-wrap:anywhere;}"
    ...
    if weather_forecast_enabled:
        ...
        yield "      <div class='forecast-current-summary' id='forecastOverall' tabindex='0' aria-label='Forecast synopsis'>--</div>"
    ...
    yield "function fetchWithTimeout(url, options = {}, timeoutMs = dashboardFetchTimeoutMs, consumeResponse = null) {"
    yield "  const controller = new AbortController();"
    yield "  let timer;"
    yield "  const timeout = new Promise((resolve, reject) => { timer = window.setTimeout(() => { controller.abort(); reject(new Error('Request timed out')); }, timeoutMs); });"
    yield "  const opts = Object.assign({}, options, { signal: controller.signal });"
    yield "  const response = fetch(url, opts).then(res => consumeResponse ? consumeResponse(res) : res);"
    yield "  return Promise.race([response, timeout]).finally(() => window.clearTimeout(timer));"
    yield "}"
    ...
    yield "    const weekday = now.toLocaleDateString('en-CA', { weekday:'short' });"
    ...
    yield "  setForecastText('forecastOverall', cur.synopsis || cur.overall || cur.forecast || '--');"
    ...
    yield " const parent = document.querySelector('.dashboard-content') || (existingRow && existingRow.parentElement) || dashboard || byGraphModal || document.body;"
    ...
    yield "  const refreshStatus = document.getElementById('dashboard_refresh_status');"
    yield "  if (refreshStatus) refreshStatus.hidden = window.__updateGaugesLastOk;"
    ...
    yield "  if (window.__updateGaugesInFlight) {"
    ...
    yield "  try {"
    yield "    d = await fetchWithTimeout(window.location.pathname + '?json_only=true&sensor_id=' + encodeURIComponent(sensorId), { cache: 'no-store' }, dashboardFetchTimeoutMs, async res => {"
    yield "      if (!res.ok) throw new Error(`http_${res.status}`);"
    yield "      return await res.json();"
    yield "    });"
    yield "    if (!d || typeof d !== 'object' || !d.values || typeof d.values !== 'object' || Array.isArray(d.values)) throw new Error('Invalid dashboard response');"
    yield "    for (const [sid, latest] of Object.entries(d.values || {})) currentValues[sid] = Object.assign({}, latest);"
    ...
    yield "  } catch (e) {"
    yield "    console.warn('updateGauges: update failed', e);"
    yield "    finishUpdateGaugesRun(finishOptions, { ok: false, error: String(e) });"
    yield "  }"
    yield ""
    yield "}"
    ...
    yield "  const dashboardContent = (firstSensorGroup && firstSensorGroup.parentElement) || document.querySelector('.dashboard-content');"
    ...
    yield "    if (el) dashboardContent.appendChild(el);"
    yield "  });"
    yield "  const switchGroup = document.getElementById('group_switches');"
    yield "  if (switchGroup) dashboardContent.appendChild(switchGroup);"
    yield "};"
