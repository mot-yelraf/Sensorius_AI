...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, expected_display_style_map=None, display_style=None, astro_payload=None, biodynamic_payload=None, weather_forecast_provider="met_no", weather_forecast_theme="garden", dashboard_background_theme="leaf"):
    ...
    yield "  const isFruit = String(cur.plant_part || '').trim().toLowerCase() === 'fruit';"
    yield "  const actionColor = isFruit ? '#4c3a7f' : color;"
    yield "  openBtn.style.background = actionColor;"
    yield "  openBtn.style.borderColor = '#27313a';"
    yield "  openBtn.style.color = isFruit ? '#fff' : textOnHex(actionColor);"
    ...
    yield "            t.textContent = text;"
    yield "            const bdColor = String(msg.to && msg.to.color || '').trim();"
    yield "            const colorMatch = bdColor.match(/^#([0-9a-f]{6})$/i);"
    yield "            if (colorMatch) {"
    yield "              const rgb = Number.parseInt(colorMatch[1], 16);"
    yield "              const red = (rgb >> 16) & 255;"
    yield "              const green = (rgb >> 8) & 255;"
    yield "              const blue = rgb & 255;"
    yield "              const luminance = (0.299 * red + 0.587 * green + 0.114 * blue) / 255;"
    yield "              t.style.backgroundColor = bdColor;"
    yield "              t.style.color = luminance > 0.62 ? '#111' : '#fff';"
    yield "            }"
    yield "            t.title = 'Click to dismiss';"
