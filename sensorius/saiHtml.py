...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, expected_display_style_map=None, display_style=None, astro_payload=None, biodynamic_payload=None, weather_forecast_provider="met_no", weather_forecast_theme="garden"):
    ...
    yield "    if (!Number.isFinite(mrToday) || !Number.isFinite(msToday) || mrToday === msToday) return [];"
    yield "    const riseMin = mrToday;"
    yield "    const setMin = msToday > riseMin ? msToday : msToday + 1440;"
    yield "    const visibleMinutes = setMin - riseMin;"
    yield "    if (visibleMinutes <= 0 || visibleMinutes >= 1440) return [];"
    yield "    const aboveAmplitude = abovePx * 0.94;"
    yield "    const belowAmplitude = belowPx * 0.9;"
    yield "    const blendStrength = 2;"
    yield "    const middleAmplitude = (aboveAmplitude + belowAmplitude) / 2;"
    yield "    const amplitudeBias = (aboveAmplitude - belowAmplitude) / (2 * Math.tanh(blendStrength));"
    yield "    const yAt = (minute) => {"
    yield "      const wave = Math.sin(Math.PI * (minute - riseMin) / visibleMinutes);"
    yield "      const mapped = wave * (middleAmplitude + (amplitudeBias * Math.tanh(blendStrength * wave)));"
    yield "      return yBase - mapped;"
    yield "    };"
    yield "    const built = [];"
