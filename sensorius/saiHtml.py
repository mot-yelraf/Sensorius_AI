...
def get_gauge_config():
    ...
    gauge_config = {
        ...
        "Equivalent CO2": {"unit": "ppm", "min": 0, "max": 3000, "ticks": [0, 400, 800, 1200, 1600, 2000, 3000], "zones": [{"strokeStyle": "#f00", "min": 0, "max": 200}, {"strokeStyle": "#ffcc00", "min": 200, "max": 400}, {"strokeStyle": "#66cc66", "min": 400, "max": 1600}, {"strokeStyle": "#ffcc00", "min": 1600, "max": 2000}, {"strokeStyle": "#f00", "min": 2000, "max": 3000}]},
        "TVOC": {"unit": "ppb", "min": 0, "max": 100, "ticks": [0, 20, 40, 60, 80, 100], "zones": [{"strokeStyle": "#6aa9df", "min": 0, "max": 100}]},
        "VOC Index": {"unit": "index", "min": 0, "max": 500, "ticks": [0, 50, 100, 150, 200, 300, 400, 500], "zones": [{"strokeStyle": "#66cc66", "min": 0, "max": 100}, {"strokeStyle": "#ffcc00", "min": 100, "max": 150}, {"strokeStyle": "#ffa500", "min": 150, "max": 200}, {"strokeStyle": "#f00", "min": 200, "max": 300}, {"strokeStyle": "#800080", "min": 300, "max": 400}, {"strokeStyle": "#800000", "min": 400, "max": 500}]},
        "NOx Index": {"unit": "index", "min": 0, "max": 500, "ticks": [0, 50, 100, 150, 200, 300, 400, 500], "zones": [{"strokeStyle": "#66cc66", "min": 0, "max": 50}, {"strokeStyle": "#ffcc00", "min": 50, "max": 100}, {"strokeStyle": "#ffa500", "min": 100, "max": 150}, {"strokeStyle": "#f00", "min": 150, "max": 200}, {"strokeStyle": "#800080", "min": 200, "max": 300}, {"strokeStyle": "#800000", "min": 300, "max": 500}]},
...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, expected_display_style_map=None, display_style=None, astro_payload=None, biodynamic_payload=None, weather_forecast_provider="met_no", weather_forecast_theme="pollinator", dashboard_background_theme="leaf", dashboard_metric_set="Pick 6", dashboard_custom_theme_style=""):
    ...
    yield "<a href='#' onclick='window.editSystemSettings && window.editSystemSettings(); return false;' title='Open General Settings' style='margin-left:8px; text-decoration:none; font-size:0.8em; vertical-align:middle;'>"
    yield from _settings_gear_svg_lines(indent="    ")
    yield "</a>"
    yield "</h2>"
    ...
    yield ".astro-box{display:flex;align-items:flex-start;justify-content:center;background:#ffffe0;border:1px solid #ccc;border-radius:10px;padding:.45rem .55rem;min-height:176px;box-sizing:border-box;position:relative;}"
    yield ".astro-box .dashboard-card-spinner{display:none;position:absolute;top:.48rem;right:.52rem;width:16px;height:16px;border-width:2px;z-index:2;background:rgba(255,255,255,.45);}"
    yield ".astro-box.card-loading .dashboard-card-spinner{display:inline-block;}"
    ...
    yield "#sunBox .astro-card{width:100%;min-width:0;height:100%;box-sizing:border-box;}"
    ...
    yield "#weatherForecastBox.forecast-scene-clear-night{--forecast-scene-bg:radial-gradient(circle at 12% 18%,#fff 0 1px,transparent 1.5px),radial-gradient(circle at 71% 29%,#fff7c2 0 1px,transparent 1.6px),radial-gradient(circle at 87% 64%,#dcecff 0 1px,transparent 1.5px),radial-gradient(circle at 34% 72%,#fff 0 1px,transparent 1.6px),radial-gradient(circle at 48% 11%,#cfe5ff 0 1px,transparent 1.5px),linear-gradient(165deg,#07172e,#172e4c 68%,#263b54);--forecast-panel:rgba(4,14,30,.58);--forecast-button:rgba(26,48,78,.9);--forecast-border:rgba(171,203,235,.56);--forecast-ink:#f4f8ff;--forecast-muted:#bdcee3;}"
    yield "#weatherForecastBox.forecast-scene-cloudy-day{--forecast-scene-bg:radial-gradient(ellipse at 20% 18%,rgba(250,252,253,.82) 0 20%,transparent 21%),radial-gradient(ellipse at 68% 31%,rgba(231,237,239,.78) 0 28%,transparent 29%),linear-gradient(160deg,#bac4c8,#95a3a9 68%,#7f8e95);--forecast-panel:rgba(238,243,245,.86);--forecast-button:#40545e;--forecast-button-ink:#fff;--forecast-border:rgba(34,54,63,.56);--forecast-ink:#14252d;--forecast-muted:#31444d;}"
    yield "#weatherForecastBox.forecast-scene-cloudy-night{--forecast-scene-bg:radial-gradient(ellipse at 22% 18%,rgba(151,164,174,.35) 0 22%,transparent 23%),radial-gradient(ellipse at 72% 30%,rgba(128,143,154,.3) 0 29%,transparent 30%),linear-gradient(160deg,#303c47,#1c2731 70%,#141e28);--forecast-panel:rgba(18,27,35,.66);--forecast-button:rgba(42,54,64,.92);--forecast-border:rgba(169,184,194,.48);--forecast-ink:#f1f5f7;--forecast-muted:#becbd1;}"
    yield "#weatherForecastBox.forecast-scene-rain-day{--forecast-scene-bg:radial-gradient(ellipse at 28% 13%,rgba(214,225,230,.56) 0 28%,transparent 29%),linear-gradient(165deg,#4b626d,#344b56 68%,#283d47);--forecast-panel:rgba(20,43,54,.82);--forecast-button:rgba(24,55,69,.96);--forecast-button-ink:#fff;--forecast-border:rgba(220,237,244,.72);--forecast-ink:#fff;--forecast-muted:#dcebf1;}"
    yield "#weatherForecastBox.forecast-scene-rain-night{--forecast-scene-bg:radial-gradient(ellipse at 28% 13%,rgba(111,130,142,.42) 0 28%,transparent 29%),linear-gradient(165deg,#273844,#17252f 68%,#101b24);--forecast-panel:rgba(10,23,31,.68);--forecast-button:rgba(29,48,59,.94);--forecast-border:rgba(144,174,188,.5);--forecast-ink:#f0f7fa;--forecast-muted:#b9cbd3;}"
    yield "#weatherForecastBox.forecast-scene-rain-day::after,#weatherForecastBox.forecast-scene-rain-night::after{inset:-18px 0 0;background:repeating-linear-gradient(105deg,transparent 0 15px,rgba(188,225,240,.5) 16px 17px,transparent 18px 29px);background-size:100% 42px;animation:forecast-rain-fall .8s linear infinite;opacity:.62;}"
    yield "@keyframes forecast-rain-fall{to{transform:translateY(42px)}}"
    yield "@media (prefers-reduced-motion:reduce){#weatherForecastBox.forecast-scene-rain-day::after,#weatherForecastBox.forecast-scene-rain-night::after{animation:none}}"
    yield "#weatherForecastBox .astro-card{position:relative;z-index:1;width:100%;min-width:0;align-items:stretch;box-sizing:border-box;text-align:left;height:100%;}"
    yield "#weatherForecastBox .astro-title{width:100%;text-align:center;color:var(--forecast-ink);text-shadow:0 1px 2px rgba(0,0,0,.24);}"
    ...
    yield ".forecast-current{border:1px solid var(--forecast-border);border-radius:8px;background:var(--forecast-panel);padding:.42rem .48rem;display:flex;flex-direction:column;gap:.3rem;min-width:0;backdrop-filter:blur(3px);}"
    ...
    yield ".moon-eclipse-line{margin:.08rem 0 0;padding:.2rem .3rem;border-top:1px solid #d5c7a8;color:#27313a;font-size:.54rem;line-height:1.2;text-align:center;}"
    yield ".moon-eclipse-line strong{color:#8a4d24;}"
    yield ".moon-eclipse-line[hidden]{display:none;}"
    ...
    yield ".bio-daylight{font-size:.58rem;line-height:1.02;text-align:center;color:#3c464d;min-height:1.02em;margin-top:-.08rem;margin-bottom:.3rem;font-variant-numeric:tabular-nums;}"
    ...
    yield ".bio-legend-root{background:#644817}.bio-legend-leaf{background:#277a00}.bio-legend-flower{background:#c4dcf8}.bio-legend-fruit{background:#4c3a7f}.bio-legend-rest{background:#d7dbe0}.bio-legend-transition{background:linear-gradient(90deg,#d64b3b 0 50%,#644817 50% 100%)}"
    ...
    yield ".bio-loading-overlay .spinner{width:28px;height:28px;border-width:3px;}"
    yield ".bio-modal.loading-month .bio-calendar-loading{display:flex;}"
    ...
    yield ".bio-modal-side{display:grid;grid-template-columns:minmax(0,1fr);grid-template-rows:auto 190px 190px;gap:.55rem;min-height:0;}"
    ...
    yield ".bio-selected-date{font-size:.8rem;font-weight:700;line-height:1.2;color:#27313a;text-align:center;}"
    yield ".bio-note-card{position:relative;border:1px solid #d4cfbf;border-radius:10px;background:#f7f8fa;padding:.75rem;display:flex;flex-direction:column;gap:.45rem;min-height:0;box-sizing:border-box;}"
    yield ".bio-summary-card.summary-loading .bio-summary-loading{display:flex;background:rgba(247,248,250,.76);}"
    yield ".bio-notes-card{height:190px;}"
    ...
    yield ".bio-note-input{width:100%;height:auto;min-height:0;flex:1 1 auto;resize:none;overflow-y:auto;border:1px solid #c8ccd0;border-radius:8px;padding:.65rem;font-size:.625rem;line-height:1.3;background:#fff;box-sizing:border-box;text-align:left;}"
    yield ".bio-summary-card .bio-summary-output{height:auto;max-height:none;min-height:78px;flex:1 1 auto;}"
    ...
    yield ".bio-note-actions{display:flex;align-items:center;justify-content:flex-start;gap:.5rem;flex-wrap:wrap;}"
    ...
    yield ".bio-note-status{font-size:.72rem;color:#4f5961;min-height:1.1em;margin-left:auto;}"
    ...
    yield ".bio-print-section-title{font-size:10pt;font-weight:800;color:#27313a;margin:.45rem 0 .25rem;text-transform:uppercase;text-align:left;}"
    ...
    yield "#sunPathCanvas{display:block;width:204px;height:72px;margin:0 auto;border:1px solid #d5c7a8;border-radius:8px;background:#dff1ff;}"
    ...
    yield ".caelus-moon-overlay{position:fixed;inset:0;z-index:10020;display:none;align-items:flex-start;justify-content:center;padding:clamp(.75rem,5vh,3rem) 1rem 1rem;background:rgba(2,10,22,.72);backdrop-filter:blur(7px);box-sizing:border-box;}"
    yield ".caelus-moon-overlay.is-open{display:flex;}"
    yield ".caelus-moon-dialog{position:relative;width:min(1180px,96vw);max-height:92vh;overflow:auto;border:1px solid rgba(126,196,193,.46);border-radius:1rem;background:linear-gradient(105deg,rgba(4,16,40,.98),rgba(7,35,51,.96));box-shadow:0 24px 70px rgba(0,0,0,.48);color:#f1f7f4;}"
    ...
    yield ".caelus-moon-dialog-title{display:flex;align-items:center;gap:.65rem;min-width:0;}"
    yield ".caelus-moon-dialog-head h2{margin:0;font:500 1.25rem Georgia,serif;}"
    yield ".caelus-moon-view-toggle{display:inline-flex;align-items:center;padding:.1rem;border:1px solid rgba(126,196,193,.72);border-radius:999px;background:rgba(19,57,74,.74);flex-shrink:0;}"
    yield ".caelus-moon-view-btn{border:0;border-radius:999px;background:transparent;color:#aec3c3;padding:.18rem .48rem;font-size:.56rem;font-weight:750;line-height:1;cursor:pointer;}"
    yield ".caelus-moon-view-btn.active{background:#319986;color:#fff;box-shadow:0 1px 3px rgba(0,0,0,.28);}"
    yield ".caelus-moon-view-btn:focus-visible{outline:2px solid #efbd65;outline-offset:2px;}"
    ...
    yield ".caelus-lunar-layout{position:relative;display:grid;grid-template-columns:1fr minmax(14rem,.72fr) 1fr;align-items:center;min-height:18rem;padding:1rem 1.35rem 4.8rem;box-sizing:border-box;}"
    ...
    yield ".caelus-lunar-side{position:relative;z-index:1;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.55rem;}"
    yield ".caelus-lunar-side-title{grid-column:1/-1;margin:0;color:rgba(235,247,240,.54);font-size:.5rem;font-weight:750;letter-spacing:.14em;text-align:center;text-transform:uppercase;}"
    yield ".caelus-lunar-step{text-align:center;color:#aec3c3;}"
    yield ".caelus-lunar-step canvas{display:block;width:clamp(2.5rem,4vw,3.8rem);height:clamp(2.5rem,4vw,3.8rem);margin:0 auto;filter:drop-shadow(0 0 11px rgba(237,229,198,.18));}"
    yield ".caelus-lunar-step small{display:block;min-height:1.3rem;margin-top:.3rem;font-size:.48rem;line-height:1.25;letter-spacing:.08em;text-transform:uppercase;}"
    yield ".caelus-lunar-step time{display:block;margin-top:.14rem;color:#efbd65;font-size:.52rem;font-weight:700;}"
    yield ".caelus-lunar-current{position:relative;z-index:2;text-align:center;align-self:start;}"
    yield ".caelus-lunar-current .caelus-live-label{margin:0 0 .2rem;color:#88ed76;font-size:.66rem;font-weight:750;letter-spacing:.16em;text-transform:uppercase;}"
    yield "#caelusCurrentMoonDisk{display:block;width:7rem;height:7rem;margin:0 auto;filter:drop-shadow(0 0 24px rgba(255,235,177,.34));}"
    yield ".caelus-lunar-current h3{margin:.15rem 0 0;font:500 1.35rem Georgia,serif;}"
    yield ".caelus-lunar-current-detail{margin:.3rem 0 0;color:#aec3c3;font-size:.67rem;}"
    yield ".caelus-lunar-local{color:#5fd3cf;font-size:.53rem;letter-spacing:.08em;text-transform:uppercase;}"
    yield ".caelus-eclipse-line{position:relative;z-index:2;grid-column:1/-1;align-self:end;justify-self:center;margin:.5rem 0 .15rem;padding:.28rem .75rem;border-top:1px solid rgba(239,189,101,.5);border-bottom:1px solid rgba(239,189,101,.24);color:#d7e5e3;font-size:.58rem;line-height:1.25;text-align:center;}"
    yield ".caelus-eclipse-line strong{color:#efbd65;}"
    yield ".caelus-lunar-footer{position:absolute;left:1.35rem;right:1.35rem;bottom:.45rem;display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:end;gap:1rem;color:#aec3c3;font-size:.58rem;}"
    yield ".caelus-lunar-timeline{display:grid;gap:.3rem;min-width:0;}"
    yield ".caelus-lunar-timeline-row{display:grid;grid-template-columns:2.5rem minmax(0,1fr);align-items:center;gap:.55rem;}"
    yield ".caelus-lunar-timeline-title{color:rgba(235,247,240,.58);font-size:.48rem;font-weight:750;letter-spacing:.13em;text-align:right;text-transform:uppercase;}"
    yield ".caelus-lunar-timeline-track{position:relative;height:1.12rem;border-top:1px solid rgba(126,196,193,.48);margin-top:.72rem;}"
    yield ".caelus-lunar-timeline-row-sun .caelus-lunar-timeline-track{border-color:rgba(239,189,101,.68);}"
    yield ".caelus-lunar-marker{position:absolute;left:var(--timeline-left,50%);top:-.72rem;display:grid;justify-items:center;min-width:4.3rem;transform:translateX(-50%);white-space:nowrap;}"
    yield ".caelus-lunar-marker::after{content:'';width:1px;height:.65rem;margin-top:.1rem;background:currentColor;opacity:.72;}"
    yield ".caelus-lunar-marker span{color:#aec3c3;font-size:.47rem;letter-spacing:.05em;text-transform:uppercase;}"
    yield ".caelus-lunar-marker strong{color:#efbd65;font-size:.58rem;}"
    yield ".caelus-lunar-timeline-row-moon .caelus-lunar-marker strong{color:#79ddd8;}"
    yield ".caelus-lunar-marker.is-start{transform:none;justify-items:start;}"
    yield ".caelus-lunar-marker.is-end{transform:translateX(-100%);justify-items:end;}"
    yield ".caelus-lunar-marker.is-unavailable{display:none;}"
    yield ".caelus-lunar-footer>small{align-self:end;color:rgba(235,247,240,.46);white-space:nowrap;}"
    yield "#moonPhaseCanvas{width:88px;height:88px;border:0;border-radius:50%;background:transparent;}"
    ...
    yield "@media (max-width: 760px){.dash-top-row{grid-template-columns:1fr;justify-items:center}.dash-left-col,.dash-right-col,.dash-side-col{display:block;width:100%;align-items:center}#sunPathCanvas{width:184px;height:66px}.astro-times{width:184px}.astro-card{min-width:120px}.dash-loc-form,.astro-box{min-height:unset}#sunBox .astro-card,#moonBox .astro-card,#bioBox .astro-card,#weatherForecastBox .astro-card,.dash-loc-form{width:206px;min-width:0}.moon-layout{grid-template-columns:minmax(0,1fr) 78px minmax(0,1fr);column-gap:.2rem}#moonPhaseCanvas{width:78px;height:78px}.moon-side{font-size:.6rem}#moonMeta{font-size:.64rem}.forecast-open-btn{min-width:128px;font-size:.62rem}.bio-day{min-height:39px;height:39px}.bio-modal .modal-body{grid-template-columns:1fr}.bio-modal-side{grid-template-columns:1fr;grid-template-rows:auto 190px 190px}.bio-note-input{height:auto;max-height:none;font-size:.47rem}.bio-summary-card .bio-summary-output{height:auto;max-height:none;min-height:68px}.bio-summary-output{font-size:.47rem}}"
    yield "@media (max-width: 760px){#sunMoon29Canvas{height:132px}.sun-moon-29-card{width:100%;min-height:unset}}"
    yield "@media (max-width:760px){.caelus-moon-overlay{padding:.35rem}.caelus-moon-dialog{width:100%;max-height:96vh}.caelus-lunar-layout{grid-template-columns:1fr;gap:.8rem;min-height:28rem;padding:1rem .65rem 5rem}.caelus-lunar-side{width:100%;max-width:34rem;margin:0 auto}.caelus-lunar-current{order:0}.caelus-lunar-side-left{order:-1}.caelus-lunar-side-right{order:1}.caelus-lunar-footer{left:.6rem;right:.6rem;grid-template-columns:1fr}.caelus-lunar-footer>small{display:none}.caelus-lunar-timeline-row{grid-template-columns:2rem minmax(0,1fr);gap:.35rem}.caelus-lunar-marker{min-width:3.7rem}.caelus-lunar-marker span{font-size:.42rem}}"
    yield "@media print{@page{size:portrait;margin:.35in}@page bio-report{size:portrait;margin:.35in}body.bio-printing *{visibility:hidden !important}body.bio-printing #bioPrintReportSheet,body.bio-printing #bioPrintReportSheet *{visibility:visible !important}body.bio-printing #bioPrintReportSheet{display:block !important;position:absolute;left:0;top:0;width:100%;padding:.08in;background:#fff;color:#000;box-sizing:border-box;page:bio-report}body.bio-printing .bio-print-calendar{gap:3px}body.bio-printing .bio-print-day{min-height:42px}body.bio-printing .bio-print-sections{gap:.35rem}body.bio-printing .bio-print-entry{break-inside:avoid;page-break-inside:avoid}}"
    yield "</style>"

    yield "<div class='dash-top-row'>"
    yield "<div class='dash-left-col'>"
    if weather_forecast_enabled:
        yield "<div class='astro-box forecast-scene-pending' id='weatherForecastBox' aria-live='polite'>"
        ...
        yield "    <div class='forecast-status' id='forecastStatus'></div>"
        yield "    <div class='forecast-current'>"
        yield "      <div class='forecast-current-summary' id='forecastOverall'>--</div>"
        ...
        yield "        <dt id='forecastPrecipLabel'>Rain</dt><dd id='forecastPrecipChance'>--</dd>"
        yield "      </dl>"
        yield "    </div>"
    ...
    yield "    <div class='bio-daylight' id='bioDaylightLine'>Hours of Daylight: --</div>"
    ...
    yield "    <div class='moon-eclipse-line' id='moonEclipse24h' hidden><strong>Eclipse next 24h:</strong> <span id='moonEclipse24hText'>--</span></div>"
    ...
    yield "    <div class='bio-card-actions'>"
    yield "      <button type='button' class='bio-open-btn' id='sunMoon29Btn' aria-label='Open 29 Day Sun/Moon Position' title='View 29 Day Sun/Moon Position'>"
    yield "        <span class='bio-open-btn-label'>29 Day Graph</span>"
    yield "      </button>"
    yield "    </div>"
    yield "  </div>"
    yield "</div>"
    ...
    yield "    <header class='caelus-moon-dialog-head'><div class='caelus-moon-dialog-title'><h2 id='caelusMoonDialogTitle'>Lunar Calendar</h2><div class='caelus-moon-view-toggle' role='group' aria-label='Lunar Calendar view mode'><button type='button' class='caelus-moon-view-btn active' id='caelusMoonViewLocal' data-moon-view='local' aria-pressed='true'>Local</button><button type='button' class='caelus-moon-view-btn' id='caelusMoonViewReference' data-moon-view='reference' aria-pressed='false'>Ref</button></div></div><button type='button' class='caelus-moon-close' id='caelusMoonClose' aria-label='Close Lunar Calendar'>×</button></header>"
    ...
    yield "      <div class='caelus-lunar-side caelus-lunar-side-left' id='caelusPreviousPhases' aria-label='Previous four Moon phases'></div>"
    yield "      <div class='caelus-lunar-current'>"
    yield "        <p class='caelus-live-label'>Moon now · live</p>"
    yield "        <canvas id='caelusCurrentMoonDisk' width='192' height='192' role='img'></canvas>"
    yield "        <h3 id='caelusCurrentMoonName'>Moon</h3>"
    yield "        <p class='caelus-lunar-current-detail'><strong id='caelusCurrentMoonIllumination'>--</strong> illuminated · lunar day <span id='caelusCurrentMoonAge'>--</span><br><span class='caelus-lunar-local' id='caelusMoonLocalView'>Observer-local orientation</span></p>"
    yield "      </div>"
    yield "      <div class='caelus-lunar-side caelus-lunar-side-right' id='caelusUpcomingPhases' aria-label='Upcoming four Moon phases'></div>"
    yield "      <p class='caelus-eclipse-line' id='caelusUpcomingEclipse'><strong>Upcoming eclipse:</strong> <span id='caelusUpcomingEclipseText'>Loading…</span></p>"
    yield "      <div class='caelus-lunar-footer'>"
    yield "        <div class='caelus-lunar-timeline' role='img' aria-label='Moon and Sun events from sunrise to the next sunrise'>"
    yield "          <div class='caelus-lunar-timeline-row caelus-lunar-timeline-row-moon'><span class='caelus-lunar-timeline-title'>Moon</span><div class='caelus-lunar-timeline-track'>"
    yield "            <span class='caelus-lunar-marker' id='caelusMoonriseMarker'><span>Moonrise</span><strong id='caelusMoonrise'>--</strong></span>"
    yield "            <span class='caelus-lunar-marker' id='caelusMoonsetMarker'><span>Moonset</span><strong id='caelusMoonset'>--</strong></span>"
    yield "          </div></div>"
    yield "          <div class='caelus-lunar-timeline-row caelus-lunar-timeline-row-sun'><span class='caelus-lunar-timeline-title'>Sun</span><div class='caelus-lunar-timeline-track'>"
    yield "            <span class='caelus-lunar-marker is-start' id='caelusSunriseMarker' style='--timeline-left:0%'><span>Sunrise</span><strong id='caelusMoonSunrise'>--</strong></span>"
    yield "            <span class='caelus-lunar-marker' id='caelusSunsetMarker'><span>Sunset</span><strong id='caelusMoonSunset'>--</strong></span>"
    yield "            <span class='caelus-lunar-marker is-end' id='caelusNextSunriseMarker' style='--timeline-left:100%'><span>Next sunrise</span><strong id='caelusNextSunrise'>--</strong></span>"
    yield "          </div></div>"
    yield "        </div>"
    yield "        <small id='caelusMoonUpdated'>Updated --</small>"
    yield "      </div>"
    yield "    </div>"
    yield "  </section>"
    yield "</div>"
    ...
    for sid, sensor_metrics in expected_gauge_map.items():
        ...
        for metric_pos, metric in enumerate(render_metrics, start=1):
            ...
            metric_title_prefix = ""
            if str(metric_display_style).lower() == "graph6hr":
                metric_title_prefix = "6hr "
            elif str(metric_display_style).lower() == "graph24hr":
                metric_title_prefix = "24hr "

            lazy_metric_visual = not default_expanded and metric_pos > 6
            lazy_visual_attr = " data-lazy-metric-visual='1'" if lazy_metric_visual else ""
            yield f"<div class='metric-container' id='{safe_id}_container' data-sensor='{sid}' data-metric='{metric}' data-display-style='{metric_display_style}'{lazy_visual_attr}>"
            yield f"<div class='metric-title'>{metric_title_prefix}{metric} ({config['unit']})</div>"

            yield "<div class='gauge-container'>"
            if not lazy_metric_visual:
    ...
    yield "<style>"
    yield ".toast-container{position:fixed; top:16px; left:50%; width:min(92vw,520px); transform:translateX(-50%); z-index:99999; display:flex; flex-direction:column; align-items:center; gap:8px; pointer-events:none}"
    yield ".toast-container.modal-pane-toast-container{position:absolute; top:12px; width:min(32rem,calc(100% - 1.5rem)); z-index:100000}"
    yield ".system-settings-content,.switch-dialog-content,.modal .split > section.panel{position:relative}"
    ...
    yield "const dashboardFetchTimeoutMs = 12000;"
    yield "const dashboardExtrasRefreshMs = 60000;"
    yield "const dashboardExtrasWarmRetryMs = 5000;"
    yield "const micrographFetchTimeoutMs = 20000;"
    yield "const micrographInflightStaleMs = 30000;"
    yield "function fetchWithTimeout(url, options = {}, timeoutMs = dashboardFetchTimeoutMs) {"
    yield "  const controller = new AbortController();"
    yield "  const timer = window.setTimeout(() => controller.abort(), timeoutMs);"
    yield "  const opts = Object.assign({}, options, { signal: controller.signal });"
    yield "  return fetch(url, opts).finally(() => window.clearTimeout(timer));"
    yield "}"
    ...
    yield "} else if (new URLSearchParams(window.location.search).has('dashboard_return')) {"
    yield "  window.history.replaceState(null, '', window.location.pathname);"
    yield "}"
    ...
    yield "let __lastAstronomyDateKey = '';"
    yield "function astronomyLocalDateKey(now) {"
    yield "  const timezoneName = String((window.caelusMoonPopupData && window.caelusMoonPopupData.timezone) || (astroData && (astroData.timezone || astroData.tz)) || '').trim();"
    yield "  try { return new Intl.DateTimeFormat('en-CA', { timeZone: timezoneName || undefined, year:'numeric', month:'2-digit', day:'2-digit' }).format(now); }"
    yield "  catch (_) { return `${now.getFullYear()}-${now.getMonth()+1}-${now.getDate()}`; }"
    yield "}"
    ...
    yield "    const weekday = now.toLocaleDateString('en-CA', { weekday:'long' });"
    yield "    ts.textContent = `${weekday} ${formatted}`;"
    yield "  }"
    ...
    yield "    const astronomyDateKey = astronomyLocalDateKey(now);"
    yield "    if (!__lastAstronomyDateKey) {"
    yield "      __lastAstronomyDateKey = astronomyDateKey;"
    yield "    } else if (astronomyDateKey !== __lastAstronomyDateKey) {"
    yield "      const previousDateKey = __lastAstronomyDateKey;"
    yield "      __lastAstronomyDateKey = astronomyDateKey;"
    yield "      if (typeof refreshCaelusAstronomy === 'function') refreshCaelusAstronomy().catch((error) => { if (__lastAstronomyDateKey === astronomyDateKey) __lastAstronomyDateKey = previousDateKey; console.error('Failed to refresh eclipse information at local midnight', error); });"
    yield "    }"
    yield "  }"
    yield "}"

    yield "function makeSmoothSkyYMapper(yBase, topY, bottomY, maxElev, minElev){"
    yield "  const rawTop = Number(topY);"
    yield "  const rawBottom = Number(bottomY);"
    yield "  const top = Number.isFinite(rawTop) ? Math.min(yBase - 1, rawTop) : yBase - 1;"
    yield "  const bottom = Number.isFinite(rawBottom) ? Math.max(yBase + 1, rawBottom) : yBase + 1;"
    yield "  const abovePx = Math.max(1, yBase - top);"
    yield "  const belowPx = Math.max(1, bottom - yBase);"
    yield "  const maxE = Math.max(1, Math.abs(Number(maxElev) || 1));"
    yield "  const minE = Math.max(1, Math.abs(Number(minElev) || 1));"
    yield "  const posScale = abovePx / maxE;"
    yield "  const negScale = belowPx / minE;"
    yield "  const bandLimit = Math.max(1, Math.min(maxE, minE));"
    yield "  const band = Math.max(0.5, Math.min(bandLimit, 12, Math.max(5, bandLimit * 0.35)));"
    yield "  const hermite = (t, y0, m0, y1, m1, span) => {"
    yield "    const tt = Math.max(0, Math.min(1, t));"
    yield "    const t2 = tt * tt;"
    yield "    const t3 = t2 * tt;"
    yield "    return ((2 * t3 - 3 * t2 + 1) * y0) + ((t3 - 2 * t2 + tt) * span * m0) + ((-2 * t3 + 3 * t2) * y1) + ((t3 - t2) * span * m1);"
    yield "  };"
    yield "  const midSlope = -((posScale + negScale) / 2);"
    yield "  return (e) => {"
    yield "    const elev = Number(e);"
    yield "    if (!Number.isFinite(elev)) return yBase;"
    yield "    if (elev >= band) return yBase - (Math.min(elev, maxE) * posScale);"
    yield "    if (elev <= -band) return yBase - (Math.max(elev, -minE) * negScale);"
    yield "    if (elev < 0) {"
    yield "      const t = (elev + band) / band;"
    yield "      return hermite(t, yBase + (band * negScale), -negScale, yBase, midSlope, band);"
    yield "    }"
    yield "    return hermite(elev / band, yBase, midSlope, yBase - (band * posScale), -posScale, band);"
    yield "  };"
    yield "}"

    yield "function pickMoonEventRaw(sameDay, nearest){"
    yield "  const dayVal = typeof sameDay === 'string' ? sameDay.trim() : '';"
    yield "  if (dayVal) return dayVal;"
    yield "  return typeof nearest === 'string' ? nearest.trim() : '';"
    yield "}"
    ...
    yield "  if (typeof setAstroCardsLoading === 'function') setAstroCardsLoading(isDashboardWarmingPayload(data));"
    ...
    yield "  };"
    yield "  const interpPointValue = (pts, minutes, key) => {"
    yield "    if (!Array.isArray(pts) || !pts.length || !Number.isFinite(minutes)) return NaN;"
    yield "    const first = Number(pts[0] && pts[0][key]);"
    yield "    if (minutes <= pts[0].m) return Number.isFinite(first) ? first : NaN;"
    yield "    for (let i = 1; i < pts.length; i++) {"
    yield "      if (minutes <= pts[i].m) {"
    yield "        const a = pts[i-1], b = pts[i];"
    yield "        const av = Number(a && a[key]);"
    yield "        const bv = Number(b && b[key]);"
    yield "        if (!Number.isFinite(av) || !Number.isFinite(bv)) return NaN;"
    yield "        const span = Math.max(1, b.m - a.m);"
    yield "        const f = (minutes - a.m) / span;"
    yield "        return av + ((bv - av) * f);"
    yield "      }"
    yield "    }"
    yield "    const last = Number(pts[pts.length - 1] && pts[pts.length - 1][key]);"
    yield "    return Number.isFinite(last) ? last : NaN;"
    yield "  };"
    yield "  const interpElev = (pts, minutes) => interpPointValue(pts, minutes, 'e');"
    yield "  const interpY = (pts, minutes) => interpPointValue(pts, minutes, 'y');"
    yield "  const sr = toMin(data && data.sunrise);"
    yield "  const ss = toMin(data && data.sunset);"
    yield "  const nn = toMin(data && data.sun_noon);"
    yield "  const mrRaw = pickMoonEventRaw(data && data.moon_rise_today, data && data.moon_rise);"
    yield "  const msRaw = pickMoonEventRaw(data && data.moon_set_today, data && data.moon_set);"
    yield "  const mr = toMin(mrRaw);"
    yield "  const ms = toMin(msRaw);"
    ...
    yield "  moonRiseEl.textContent = fmtSun(mrRaw);"
    yield "  moonSetEl.textContent = fmtSun(msRaw);"
    ...
    yield "  const yForElev = makeSmoothSkyYMapper(yBase, padY + 2, c.height - padY - 2, maxElev, minElev);"
    yield "  const xForMin = (m) => padX + ((Math.max(0, Math.min(1440, m)) / 1440) * w);"
    yield "  const curveHermite = (t, y0, s0, y1, s1, span) => {"
    yield "    const tt = Math.max(0, Math.min(1, t));"
    yield "    const t2 = tt * tt;"
    yield "    const t3 = t2 * tt;"
    yield "    return ((2 * t3 - 3 * t2 + 1) * y0) + ((t3 - 2 * t2 + tt) * span * s0) + ((-2 * t3 + 3 * t2) * y1) + ((t3 - t2) * span * s1);"
    yield "  };"
    yield "  const buildSunCurveKeys = () => {"
    ...
    yield "    const prevSunset = ss - 1440;"
    yield "    const nextSunrise = sr + 1440;"
    yield "    const prevTrough = prevSunset + ((sr - prevSunset) / 2);"
    yield "    const nextTrough = ss + ((nextSunrise - ss) / 2);"
    yield "    const topY = yBase - abovePx;"
    yield "    const bottomY = yBase + belowPx;"
    yield "    const riseSlope = -Math.min((abovePx * Math.PI) / (2 * Math.max(1, peakMin - sr)), (belowPx * Math.PI) / (2 * Math.max(1, sr - prevTrough)));"
    yield "    const setSlope = Math.min((abovePx * Math.PI) / (2 * Math.max(1, ss - peakMin)), (belowPx * Math.PI) / (2 * Math.max(1, nextTrough - ss)));"
    yield "    return ["
    yield "      {m: prevSunset, y: yBase, s: setSlope},"
    yield "      {m: prevTrough, y: bottomY, s: 0},"
    yield "      {m: sr, y: yBase, s: riseSlope},"
    yield "      {m: peakMin, y: topY, s: 0},"
    yield "      {m: ss, y: yBase, s: setSlope},"
    yield "      {m: nextTrough, y: bottomY, s: 0},"
    yield "      {m: nextSunrise, y: yBase, s: riseSlope},"
    yield "    ];"
    yield "  };"
    yield "  const sunKeys = buildSunCurveKeys();"
    yield "  const sunYForMin = (m) => {"
    yield "    if (!sunKeys.length) return yBase;"
    yield "    const minuteRaw = Number(m);"
    yield "    const minute = Number.isFinite(minuteRaw) ? Math.max(0, Math.min(1440, minuteRaw)) : 0;"
    yield "    if (minute <= sunKeys[0].m) return sunKeys[0].y;"
    yield "    for (let i = 0; i < sunKeys.length - 1; i++) {"
    yield "      const a = sunKeys[i];"
    yield "      const b = sunKeys[i + 1];"
    yield "      if (minute <= b.m) {"
    yield "        const span = Math.max(1, b.m - a.m);"
    yield "        return curveHermite((minute - a.m) / span, a.y, a.s, b.y, b.s, span);"
    yield "      }"
    yield "    }"
    yield "    return sunKeys[sunKeys.length - 1].y;"
    yield "  };"
    yield "  const buildSunDisplayPoints = () => {"
    yield "    if (!sunKeys.length) return [];"
    yield "    const built = [];"
    yield "    for (let m = 0; m <= 1440; m += 5) {"
    yield "      built.push({m, y: sunYForMin(m)});"
    yield "    }"
    yield "    return built;"
    yield "  };"
    yield "  const smoothSampledElevationPath = (pts, stepMin) => {"
    yield "    const sorted = Array.isArray(pts) ? pts.filter((p) => Number.isFinite(p.m) && Number.isFinite(p.e)).sort((a,b) => a.m - b.m) : [];"
    yield "    if (sorted.length < 2) return [];"
    yield "    const step = Math.max(1, Number(stepMin) || 5);"
    yield "    const yVals = sorted.map((p) => yForElev(p.e));"
    yield "    const slopeAt = (idx) => {"
    yield "      if (idx > 0 && idx < sorted.length - 1) {"
    yield "        const dyPrev = yVals[idx] - yVals[idx - 1];"
    yield "        const dyNext = yVals[idx + 1] - yVals[idx];"
    yield "        if ((dyPrev < 0 && dyNext > 0) || (dyPrev > 0 && dyNext < 0)) return 0;"
    yield "      }"
    yield "      const prev = sorted[Math.max(0, idx - 1)];"
    yield "      const next = sorted[Math.min(sorted.length - 1, idx + 1)];"
    yield "      const span = Math.max(1, next.m - prev.m);"
    yield "      return ((yVals[Math.min(sorted.length - 1, idx + 1)] - yVals[Math.max(0, idx - 1)]) / span) * 0.65;"
    yield "    };"
    yield "    const slopes = sorted.map((_p, idx) => slopeAt(idx));"
    yield "    const built = [];"
    yield "    for (let i = 0; i < sorted.length - 1; i++) {"
    yield "      const a = sorted[i];"
    yield "      const b = sorted[i + 1];"
    yield "      const span = Math.max(1, b.m - a.m);"
    yield "      const start = i === 0 ? a.m : Math.ceil(a.m / step) * step;"
    yield "      const end = b.m;"
    yield "      for (let m = start; m < end; m += step) {"
    yield "        const t = (m - a.m) / span;"
    yield "        built.push({m, y: curveHermite(t, yVals[i], slopes[i], yVals[i + 1], slopes[i + 1], span)});"
    yield "      }"
    yield "    }"
    yield "    const last = sorted[sorted.length - 1];"
    yield "    built.push({m: last.m, y: yVals[yVals.length - 1]});"
    yield "    return built;"
    yield "  };"
    yield "  const softenMoonLowerExtrema = (pts) => {"
    yield "    const sorted = (Array.isArray(pts) ? pts : []).map((p) => ({m: Number(p && p.m), y: Number(p && p.y)})).filter((p) => Number.isFinite(p.m) && Number.isFinite(p.y)).sort((a,b) => a.m - b.m);"
    yield "    if (sorted.length < 5) return sorted;"
    yield "    const rounded = sorted.map((p) => ({m: p.m, y: p.y}));"
    yield "    const lastIdx = sorted.length - 1;"
    yield "    const lowerExtrema = [];"
    yield "    for (let i = 2; i < lastIdx - 1; i++) {"
    yield "      const y = sorted[i].y;"
    yield "      if (y <= yBase + 3) continue;"
    yield "      const leftY = sorted[i - 1].y;"
    yield "      const rightY = sorted[i + 1].y;"
    yield "      if (y >= leftY && y >= rightY && (y > leftY || y > rightY)) {"
    yield "        const prevIdx = lowerExtrema.length ? lowerExtrema[lowerExtrema.length - 1] : -1;"
    yield "        if (prevIdx >= 0 && sorted[i].m - sorted[prevIdx].m < 32) {"
    yield "          if (y > sorted[prevIdx].y) lowerExtrema[lowerExtrema.length - 1] = i;"
    yield "        } else {"
    yield "          lowerExtrema.push(i);"
    yield "        }"
    yield "      }"
    yield "    }"
    yield "    lowerExtrema.forEach((idx) => {"
    yield "      const apex = sorted[idx];"
    yield "      let leftHorizon = null;"
    yield "      let rightHorizon = null;"
    yield "      for (let j = idx; j >= 0; j--) {"
    yield "        if (sorted[j].y <= yBase + 1.5) { leftHorizon = sorted[j].m; break; }"
    yield "      }"
    yield "      for (let j = idx; j <= lastIdx; j++) {"
    yield "        if (sorted[j].y <= yBase + 1.5) { rightHorizon = sorted[j].m; break; }"
    yield "      }"
    yield "      const leftRoom = Number.isFinite(leftHorizon) ? apex.m - leftHorizon : 90;"
    yield "      const rightRoom = Number.isFinite(rightHorizon) ? rightHorizon - apex.m : 90;"
    yield "      const halfWindow = Math.max(48, Math.min(420, leftRoom * 0.88, rightRoom * 0.88));"
    yield "      const leftM = Math.max(sorted[0].m, apex.m - halfWindow);"
    yield "      const rightM = Math.min(sorted[lastIdx].m, apex.m + halfWindow);"
    yield "      if (apex.m - leftM < 18 || rightM - apex.m < 18) return;"
    yield "      const leftY = interpY(sorted, leftM);"
    yield "      const rightY = interpY(sorted, rightM);"
    yield "      if (!Number.isFinite(leftY) || !Number.isFinite(rightY)) return;"
    yield "      rounded.forEach((p) => {"
    yield "        if (p.m < leftM || p.m > rightM) return;"
    yield "        if (p.m <= apex.m) {"
    yield "          const span = Math.max(1, apex.m - leftM);"
    yield "          const eased = Math.sin(((p.m - leftM) / span) * Math.PI * 0.5);"
    yield "          p.y = leftY + ((apex.y - leftY) * eased);"
    yield "        } else {"
    yield "          const span = Math.max(1, rightM - apex.m);"
    yield "          const eased = Math.cos(((p.m - apex.m) / span) * Math.PI * 0.5);"
    yield "          p.y = rightY + ((apex.y - rightY) * eased);"
    yield "        }"
    yield "      });"
    yield "    });"
    yield "    return rounded;"
    yield "  };"
    yield "  const buildMoonRiseSetDisplayPoints = () => {"
    yield "    if (!Number.isFinite(mr) || !Number.isFinite(ms) || mr === ms) return [];"
    yield "    const riseMin = mr;"
    yield "    const setMin = ms > riseMin ? ms : ms + 1440;"
    yield "    const visibleMinutes = setMin - riseMin;"
    yield "    if (visibleMinutes <= 0 || visibleMinutes >= 1440) return [];"
    yield "    const amplitude = Math.min(abovePx * 0.94, belowPx * 0.9);"
    yield "    const yAt = (minute) => {"
    yield "      const wave = Math.sin(Math.PI * (minute - riseMin) / visibleMinutes);"
    yield "      return yBase - (amplitude * wave);"
    yield "    };"
    yield "    const built = [];"
    yield "    for (let m = 0; m <= 1440; m += 2) {"
    yield "      built.push({m, y: yAt(m)});"
    yield "    }"
    yield "    return built;"
    yield "  };"
    yield "  const buildMoonDisplayPoints = () => {"
    yield "    const riseSet = buildMoonRiseSetDisplayPoints();"
    yield "    if (riseSet.length >= 2) return riseSet;"
    yield "    const sampled = smoothSampledElevationPath(moonPoints, 2);"
    yield "    if (sampled.length >= 2) return sampled;"
    yield "    return moonPoints;"
    yield "  };"
    ...
    yield "    const xy = pts.map((p) => {"
    yield "      const yRaw = Number(p && p.y);"
    yield "      const eRaw = Number(p && p.e);"
    yield "      const y = Number.isFinite(yRaw) ? yRaw : yForElev(eRaw);"
    yield "      return {x: xForMin(p && p.m), y};"
    yield "    }).filter((p) => Number.isFinite(p.x) && Number.isFinite(p.y));"
    yield "    if (xy.length < 2) { ctx.restore(); return false; }"
    yield "    ctx.beginPath();"
    yield "    ctx.moveTo(xy[0].x, xy[0].y);"
    yield "    const tangentAt = (idx) => {"
    yield "      const cur = xy[idx];"
    yield "      const prev = xy[Math.max(0, idx - 1)];"
    yield "      const next = xy[Math.min(xy.length - 1, idx + 1)];"
    yield "      let ty = (next.y - prev.y) * 0.38;"
    yield "      if (idx > 0 && idx < xy.length - 1) {"
    yield "        const dyPrev = cur.y - prev.y;"
    yield "        const dyNext = next.y - cur.y;"
    yield "        if ((dyPrev < 0 && dyNext > 0) || (dyPrev > 0 && dyNext < 0)) ty = 0;"
    yield "      }"
    yield "      return {x: (next.x - prev.x) * 0.38, y: ty};"
    yield "    };"
    yield "    const tangents = xy.map((_p, idx) => tangentAt(idx));"
    yield "    for (let i = 0; i < xy.length - 1; i++) {"
    yield "      const p0 = xy[i];"
    yield "      const p1 = xy[i + 1];"
    yield "      const t0 = tangents[i];"
    yield "      const t1 = tangents[i + 1];"
    yield "      ctx.bezierCurveTo("
    yield "        p0.x + (t0.x / 3), p0.y + (t0.y / 3),"
    yield "        p1.x - (t1.x / 3), p1.y - (t1.y / 3),"
    yield "        p1.x, p1.y"
    yield "      );"
    yield "    }"
    yield "    ctx.stroke();"
    ...
    yield "  const moonDisplayPoints = buildMoonDisplayPoints();"
    yield "  const drewSun = drawPath(sunDisplayPoints, '#f3d34a', 2.1, []);"
    yield "  if (!drewSun && data && data.ok && Number.isFinite(sr) && Number.isFinite(ss) && sr < ss){"
    yield "    ctx.strokeStyle = '#f3d34a';"
    yield "    ctx.lineWidth = 2.1;"
    yield "    ctx.lineCap = 'round';"
    yield "    ctx.beginPath();"
    yield "    for (let m = 0; m <= 1440; m += 10){"
    yield "      const x = xForMin(m);"
    yield "      const y = sunYForMin(m);"
    yield "      if (m === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);"
    yield "    }"
    yield "    ctx.stroke();"
    yield "  }"
    yield "  drawPath(moonDisplayPoints, '#69bdf2', 1.65, []);"
    yield "  const now = new Date();"
    yield "  const curMin = (now.getHours() * 60) + now.getMinutes();"
    yield "  const xNow = xForMin(curMin);"
    yield "  const sunYNow = interpY(sunDisplayPoints, curMin);"
    yield "  const yNow = Number.isFinite(sunYNow) ? sunYNow : ((Number.isFinite(sr) && Number.isFinite(ss) && sr < ss) ? sunYForMin(curMin) : yBase);"
    ...
    yield "  ctx.lineWidth = 1;"
    yield "  ctx.stroke();"
    yield "  const moonYNow = interpY(moonDisplayPoints, curMin);"
    yield "  if (Number.isFinite(moonYNow)){"
    yield "    const yMoonNow = moonYNow;"
    yield "    ctx.save();"
    yield "    ctx.shadowColor = 'rgba(0,0,0,.35)';"
    yield "    ctx.shadowBlur = 2;"
    yield "    ctx.shadowOffsetY = 0.6;"
    yield "    ctx.fillStyle = '#fff8df';"
    yield "    ctx.beginPath();"
    yield "    ctx.arc(xNow, yMoonNow, 4.2, 0, Math.PI*2);"
    yield "    ctx.fill();"
    yield "    ctx.shadowColor = 'transparent';"
    yield "    ctx.strokeStyle = '#2e78a9';"
    yield "    ctx.lineWidth = 1.35;"
    yield "    ctx.stroke();"
    yield "    ctx.fillStyle = '#d9edff';"
    yield "    ctx.beginPath();"
    yield "    ctx.arc(xNow - 1.1, yMoonNow - 1.1, 1.1, 0, Math.PI*2);"
    yield "    ctx.fill();"
    yield "    ctx.restore();"
    yield "  }"
    ...
    yield "  const yForElev = makeSmoothSkyYMapper(yBase, padT + 2, padT + graphH - 2, maxElev, minElev);"
    ...
    yield "    ctx.lineJoin = 'round';"
    yield "    ctx.beginPath();"
    yield "    const xy = sorted.map((p) => ({x: xForMin(p.x), y: yForElev(p.e)})).filter((p) => Number.isFinite(p.x) && Number.isFinite(p.y));"
    yield "    if (xy.length < 2) { ctx.restore(); return; }"
    yield "    ctx.moveTo(xy[0].x, xy[0].y);"
    ...
    yield ""
    yield "function caelusMoonIsOpen(){"
    yield "  const overlay = document.getElementById('caelusMoonOverlay');"
    yield "  return !!(overlay && overlay.classList.contains('is-open'));"
    yield "}"
    yield "function setCaelusMoonText(id, value){"
    yield "  const element = document.getElementById(id);"
    yield "  if (element) element.textContent = value == null || value === '' ? '--' : String(value);"
    yield "}"
    yield "function formatCaelusEclipse(event){"
    yield "  if (!event || typeof event !== 'object') return '';"
    yield "  const kind = String(event.kind || 'Eclipse');"
    yield "  const date = String(event.date || '');"
    yield "  const starts = String(event.starts || event.time || '');"
    yield "  const ends = String(event.ends || '');"
    yield "  const timeRange = starts && ends ? `${starts}–${ends}` : starts;"
    yield "  return [kind, date, timeRange].filter(Boolean).join(' · ');"
    yield "}"
    yield "function renderDashboardEclipse(moon){"
    yield "  const line = document.getElementById('moonEclipse24h');"
    yield "  const event = moon && moon.eclipse_next_24h;"
    yield "  if (!line) return;"
    yield "  line.hidden = !event;"
    yield "  if (event) setCaelusMoonText('moonEclipse24hText', formatCaelusEclipse(event));"
    yield "}"
    yield "function renderUpcomingEclipse(moon){"
    yield "  const eclipses = moon && Array.isArray(moon.next_eclipses) ? moon.next_eclipses : [];"
    yield "  setCaelusMoonText('caelusUpcomingEclipseText', eclipses.length ? formatCaelusEclipse(eclipses[0]) : 'No visible eclipse in the next 12 months');"
    yield "}"
    yield "function positionCaelusTimelineMarker(markerId, eventAt, startAt, endAt){"
    yield "  const marker = document.getElementById(markerId);"
    yield "  if (!marker) return;"
    yield "  const eventMs = Date.parse(String(eventAt || ''));"
    yield "  const startMs = Date.parse(String(startAt || ''));"
    yield "  const endMs = Date.parse(String(endAt || ''));"
    yield "  const available = Number.isFinite(eventMs) && Number.isFinite(startMs) && Number.isFinite(endMs) && endMs > startMs && eventMs >= startMs && eventMs <= endMs;"
    yield "  marker.classList.toggle('is-unavailable', !available);"
    yield "  if (!available) return;"
    yield "  const percent = Math.max(0, Math.min(100, ((eventMs - startMs) / (endMs - startMs)) * 100));"
    yield "  marker.style.setProperty('--timeline-left', `${percent.toFixed(2)}%`);"
    yield "}"
    yield "function renderCaelusPhaseSide(containerId, title, phases){"
    yield "  const container = document.getElementById(containerId);"
    yield "  if (!container) return;"
    yield "  container.replaceChildren();"
    yield "  const heading = document.createElement('p');"
    yield "  heading.className = 'caelus-lunar-side-title';"
    yield "  heading.textContent = title;"
    yield "  container.appendChild(heading);"
    yield "  (Array.isArray(phases) ? phases.slice(0, 4) : []).forEach((phase) => {"
    yield "    const step = document.createElement('div');"
    yield "    step.className = 'caelus-lunar-step';"
    yield "    const canvas = document.createElement('canvas');"
    yield "    canvas.width = 96;"
    yield "    canvas.height = 96;"
    yield "    canvas.dataset.phaseMoon = '';"
    yield "    canvas.dataset.phaseIndex = String(phase.index == null ? '' : phase.index);"
    yield "    const name = document.createElement('small');"
    yield "    name.textContent = phase.name || 'Moon phase';"
    yield "    const date = document.createElement('time');"
    yield "    date.dateTime = phase.representative_date || '';"
    yield "    date.textContent = phase.date_label || phase.representative_date || '--';"
    yield "    step.append(canvas, name, date);"
    yield "    container.appendChild(step);"
    yield "    if (window.CaelusMoon) window.CaelusMoon.renderMoonDisk(canvas, phase);"
    yield "  });"
    yield "}"
    yield "function renderCaelusMoonPopup(moon){"
    yield "  if (!moon || typeof moon !== 'object') return;"
    yield "  window.caelusMoonPopupData = moon;"
    yield "  renderCaelusPhaseSide('caelusPreviousPhases', 'Previous phases', moon.previous_phases);"
    yield "  renderCaelusPhaseSide('caelusUpcomingPhases', 'Upcoming phases', moon.upcoming_phases);"
    yield "  const currentCanvas = document.getElementById('caelusCurrentMoonDisk');"
    yield "  if (currentCanvas && window.CaelusMoon) window.CaelusMoon.renderMoonDisk(currentCanvas, moon);"
    yield "  setCaelusMoonText('caelusCurrentMoonName', moon.name || 'Moon');"
    yield "  setCaelusMoonText('caelusCurrentMoonIllumination', moon.illumination == null ? '--' : `${moon.illumination}%`);"
    yield "  setCaelusMoonText('caelusCurrentMoonAge', moon.age_days);"
    yield "  updateCaelusMoonViewLabel(moon);"
    yield "  renderDashboardEclipse(moon);"
    yield "  renderUpcomingEclipse(moon);"
    yield "  setCaelusMoonText('caelusMoonSunrise', moon.sunrise);"
    yield "  setCaelusMoonText('caelusMoonSunset', moon.sunset);"
    yield "  setCaelusMoonText('caelusNextSunrise', moon.next_sunrise);"
    yield "  setCaelusMoonText('caelusMoonrise', moon.timeline_moonrise);"
    yield "  setCaelusMoonText('caelusMoonset', moon.timeline_moonset);"
    yield "  positionCaelusTimelineMarker('caelusSunsetMarker', moon.timeline_sunset_at, moon.timeline_start_at, moon.timeline_end_at);"
    yield "  positionCaelusTimelineMarker('caelusMoonriseMarker', moon.timeline_moonrise_at, moon.timeline_start_at, moon.timeline_end_at);"
    yield "  positionCaelusTimelineMarker('caelusMoonsetMarker', moon.timeline_moonset_at, moon.timeline_start_at, moon.timeline_end_at);"
    yield "  const updated = String(moon.updated_at || '');"
    yield "  setCaelusMoonText('caelusMoonUpdated', updated.length >= 16 ? `Updated ${updated.slice(11, 16)} UTC` : 'Updated now');"
    yield "  setCaelusMoonText('caelusMoonStatus', 'Observer-local phase timeline');"
    yield "}"
    yield "let caelusAstronomyPromise = null;"
    yield "function loadCaelusAstronomy(){"
    yield "  if (!caelusAstronomyPromise) {"
    yield "    caelusAstronomyPromise = fetch('/api/weather-forecast-app/astronomy', {cache:'no-store'}).then((response) => { if (!response.ok) throw new Error(`astronomy_${response.status}`); return response.json(); }).catch((error) => { caelusAstronomyPromise = null; throw error; });"
    yield "  }"
    yield "  return caelusAstronomyPromise;"
    yield "}"
    yield "async function refreshCaelusAstronomy(){"
    yield "  caelusAstronomyPromise = null;"
    yield "  const moon = await loadCaelusAstronomy();"
    yield "  renderDashboardEclipse(moon);"
    yield "  renderUpcomingEclipse(moon);"
    yield "  if (caelusMoonIsOpen()) renderCaelusMoonPopup(moon);"
    yield "  return moon;"
    yield "}"
    yield "async function openCaelusMoonPhases(){"
    yield "  const overlay = document.getElementById('caelusMoonOverlay');"
    yield "  const dialog = document.getElementById('caelusMoonDialog');"
    yield "  if (!overlay || !dialog) return;"
    yield "  overlay.classList.add('is-open');"
    yield "  overlay.setAttribute('aria-hidden', 'false');"
    yield "  setCaelusMoonText('caelusMoonStatus', 'Loading observer-local Moon phases…');"
    yield "  try { if (dialog) dialog.focus({preventScroll:true}); } catch (_) { dialog.focus(); }"
    yield "  try {"
    yield "    renderCaelusMoonPopup(await loadCaelusAstronomy());"
    yield "  } catch (error) {"
    yield "    console.error('Failed to load Caelus Moon phases', error);"
    yield "    setCaelusMoonText('caelusMoonStatus', 'Moon phase timeline unavailable');"
    yield "  }"
    yield "}"
    ...
    yield "document.addEventListener('DOMContentLoaded', function(){ refreshCaelusAstronomy().catch((error) => console.error('Failed to load eclipse information', error)); });"
    yield ""
    ...
    yield "  if (target.closest('#caelusMoonClose') || target.id === 'caelusMoonOverlay') {"
    yield "    ev.preventDefault();"
    yield "    closeCaelusMoonPhases();"
    yield "    return;"
    yield "  }"
    ...
    yield "  const target = ev.target instanceof Element ? ev.target : null;"
    yield "  if (ev.key === 'Escape' && caelusMoonIsOpen()) {"
    yield "    ev.preventDefault();"
    yield "    closeCaelusMoonPhases();"
    yield "    return;"
    yield "  }"
    ...
    yield "  if (window.CaelusMoon && typeof window.CaelusMoon.getViewMode === 'function') return window.CaelusMoon.getViewMode();"
    yield "  try { return window.localStorage.getItem('sensorius.moonViewMode') === 'reference' ? 'reference' : 'local'; } catch (_) {}"
    yield "  return 'local';"
    yield "}"
    yield ""
    yield "function updateCaelusMoonViewLabel(moon){"
    yield "  if (getMoonViewMode() === 'reference'){ setCaelusMoonText('caelusMoonLocalView', 'Reference orientation · lunar north up'); return; }"
    yield "  const altitude = Number(moon && moon.moon_altitude);"
    yield "  setCaelusMoonText('caelusMoonLocalView', Number.isFinite(altitude) ? `Observer-local orientation · ${altitude}° altitude${altitude < 0 ? ' (below horizon)' : ''}` : 'Observer-local orientation');"
    yield "}"
    yield ""
    yield "function setMoonViewMode(mode){"
    yield "  const isReference = mode === 'reference';"
    yield "  document.querySelectorAll('[data-moon-view]').forEach((button) => {"
    yield "    const active = (button.getAttribute('data-moon-view') === 'reference') === isReference;"
    yield "    button.classList.toggle('active', active);"
    yield "    button.setAttribute('aria-pressed', active ? 'true' : 'false');"
    yield "  });"
    yield "  if (window.CaelusMoon && typeof window.CaelusMoon.setViewMode === 'function') window.CaelusMoon.setViewMode(isReference ? 'reference' : 'local');"
    yield "  else { try { window.localStorage.setItem('sensorius.moonViewMode', isReference ? 'reference' : 'local'); } catch (_) {} }"
    yield "  updateCaelusMoonViewLabel(window.caelusMoonPopupData);"
    yield "}"
    yield ""
    yield "const dashboardMoonSurfaceImage = new Image();"
    yield "dashboardMoonSurfaceImage.addEventListener('load', function(){ drawMoonPhase(astroData); });"
    yield "dashboardMoonSurfaceImage.src = '/ui_static/weather_forecast/moon-surface.png?v=1';"
    yield ""
    ...
    yield "  };"
    yield "  const setMoonNextPhaseLabel = (rawLabel) => {"
    yield "    const label = fmtRaw(rawLabel) === '--' ? 'Next Phase' : fmtRaw(rawLabel);"
    yield "    nextPhaseLabelEl.replaceChildren();"
    yield "    const namedFullMoon = label !== 'New Moon' && /\\sMoon$/.test(label);"
    yield "    if (!namedFullMoon){"
    yield "      nextPhaseLabelEl.textContent = label;"
    yield "      return;"
    yield "    }"
    yield "    const descriptor = label.replace(/\\s+Moon$/, '');"
    yield "    nextPhaseLabelEl.append(document.createTextNode(descriptor));"
    yield "    nextPhaseLabelEl.append(document.createElement('br'));"
    yield "    nextPhaseLabelEl.append(document.createTextNode('Moon'));"
    yield "  };"
    ...
    yield "  const moonPhaseRiseRaw = pickMoonEventRaw(data && data.moon_rise_today, data && data.moon_rise);"
    yield "  const moonPhaseSetRaw = pickMoonEventRaw(data && data.moon_set_today, data && data.moon_set);"
    yield "  const sx = limbStrength;"
    yield "  const sz = -Math.cos(phaseAngle);"
    yield "  const phaseCanvas = document.createElement('canvas');"
    ...
    yield "  const pix = image.data;"
    yield "  let surfacePixels = null;"
    yield "  if (dashboardMoonSurfaceImage.complete && dashboardMoonSurfaceImage.naturalWidth > 0){"
    yield "    const surfaceCanvas = document.createElement('canvas');"
    yield "    surfaceCanvas.width = w; surfaceCanvas.height = h;"
    yield "    const surfaceCtx = surfaceCanvas.getContext('2d', {willReadFrequently:true});"
    yield "    if (surfaceCtx){"
    yield "      surfaceCtx.drawImage(dashboardMoonSurfaceImage, 0, 0, w, h);"
    yield "      surfacePixels = surfaceCtx.getImageData(0, 0, w, h).data;"
    yield "    }"
    yield "  }"
    ...
    yield "      const litMix = Math.pow(blend, 0.82);"
    yield "      const rim = Math.pow(Math.max(0, dz), 0.65);"
    yield "      if (surfacePixels){"
    yield "        const textureRadius = Math.min(w, h) * 0.44;"
    yield "        const sourceX = Math.max(0, Math.min(w - 1, Math.round(x + dx * textureRadius)));"
    yield "        const sourceY = Math.max(0, Math.min(h - 1, Math.round(y + dy * textureRadius)));"
    yield "        const sourceOff = (sourceY * w + sourceX) * 4;"
    yield "        const brightness = 0.035 + litMix * (0.9 + dz * 0.065);"
    yield "        pix[off+0] = Math.min(255, Math.round(surfacePixels[sourceOff] * brightness));"
    yield "        pix[off+1] = Math.min(255, Math.round(surfacePixels[sourceOff+1] * brightness));"
    yield "        pix[off+2] = Math.min(255, Math.round(surfacePixels[sourceOff+2] * brightness + (1 - litMix) * 5));"
    yield "      } else {"
    yield "        const darkR = 74, darkG = 78, darkB = 86;"
    yield "        const litR = 244, litG = 242, litB = 234;"
    ...
    yield "      }"
    yield "      const edgeDistance = (1 - Math.sqrt(rr)) * r;"
    yield "      pix[off+3] = Math.round(255 * Math.max(0, Math.min(1, edgeDistance / 1.35)));"
    yield "    }"
    yield "  }"
    yield "  phaseCtx.putImageData(image, 0, 0);"
    ...
    yield "  phaseCtx.clip();"
    yield "  for (const m of (surfacePixels ? [] : maria)) {"
    ...
    yield "  riseEl.textContent = fmtMoonTime(moonPhaseRiseRaw);"
    yield "  setEl.textContent = fmtMoonTime(moonPhaseSetRaw);"
    yield "  litEl.textContent = litPct;"
    yield "  setMoonNextPhaseLabel(data.moon_next_phase_label);"
    ...
    yield "});"
    yield "window.addEventListener('sensorius:moon-view-change', function(ev){"
    yield "  const mode = ev.detail && ev.detail.mode === 'reference' ? 'reference' : 'local';"
    yield "  document.querySelectorAll('[data-moon-view]').forEach((button) => { const active = button.getAttribute('data-moon-view') === mode; button.classList.toggle('active', active); button.setAttribute('aria-pressed', active ? 'true' : 'false'); });"
    yield "  updateCaelusMoonViewLabel(window.caelusMoonPopupData);"
    yield "  if (typeof astroData !== 'undefined') drawMoonPhase(astroData);"
    yield "});"
    yield "document.addEventListener('DOMContentLoaded', function(){ setMoonViewMode(getMoonViewMode()); });"
    yield ""
    ...
    yield "}"
    yield "function forecastCurrentTemperature(data){"
    yield "  const readings = (data && data.current_readings && typeof data.current_readings === 'object') ? data.current_readings : null;"
    yield "  if (!readings || !readings.ok || !readings.sensor_id) return '';"
    yield "  const metric = String((data && data.unit_system) || '').toLowerCase() === 'metric';"
    yield "  const rawValue = metric ? readings.temperature_c : readings.temperature_f;"
    yield "  if (rawValue === null || rawValue === undefined || rawValue === '') return '';"
    yield "  const value = Number(rawValue);"
    yield "  return Number.isFinite(value) ? `${value.toFixed(1)}${metric ? '°C' : '°F'}` : '';"
    yield "}"
    yield "function forecastCurrentHumidity(data){"
    yield "  const readings = (data && data.current_readings && typeof data.current_readings === 'object') ? data.current_readings : null;"
    yield "  if (!readings || !readings.ok || !readings.sensor_id) return '';"
    yield "  if (readings.humidity === null || readings.humidity === undefined || readings.humidity === '') return '';"
    yield "  const value = Number(readings.humidity);"
    yield "  return Number.isFinite(value) ? `${value.toFixed(1)}%` : '';"
    yield "}"
    yield "function forecastReadingAndRange(currentValue, forecastRange){"
    yield "  const range = String(forecastRange || '').trim() || '--';"
    yield "  return currentValue ? `${currentValue} : ${range}` : range;"
    yield "}"
    ...
    yield "}"
    yield "function forecastPrecipitationKind(data, cur){"
    yield "  const rows = Array.isArray(data && data.hourly) ? data.hourly : [];"
    yield "  const text = [cur && (cur.overall || cur.forecast), ...rows.map((row) => row && row.symbol)].join(' ').toLowerCase();"
    yield "  return /snow|sleet/.test(text) ? 'Snow' : 'Rain';"
    yield "}"
    yield "function setForecastPrecipitation(data, cur){"
    yield "  const label = document.getElementById('forecastPrecipLabel');"
    yield "  const rawChance = cur && cur.precip_probability;"
    yield "  const chance = Number(rawChance);"
    yield "  if (label) label.textContent = forecastPrecipitationKind(data, cur);"
    yield "  setForecastText('forecastPrecipChance', rawChance !== null && rawChance !== undefined && rawChance !== '' && Number.isFinite(chance) ? `${Math.round(Math.max(0, Math.min(100, chance)))}% chance` : '--');"
    yield "}"
    yield "function forecastClockMinutes(value){"
    yield "  const match = String(value || '').match(/(?:T|^)(\\d{1,2}):(\\d{2})/);"
    yield "  if (!match) return NaN;"
    yield "  return (Number(match[1]) * 60) + Number(match[2]);"
    yield "}"
    yield "function forecastStationMinutes(data){"
    yield "  const timezoneName = String((data && data.location && data.location.timezone) || '').trim();"
    yield "  if (!timezoneName) return NaN;"
    yield "  try {"
    yield "    const parts = new Intl.DateTimeFormat('en-US', {timeZone:timezoneName,hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).formatToParts(new Date());"
    yield "    const hour = Number((parts.find((part) => part.type === 'hour') || {}).value);"
    yield "    const minute = Number((parts.find((part) => part.type === 'minute') || {}).value);"
    yield "    return Number.isFinite(hour) && Number.isFinite(minute) ? ((hour % 24) * 60) + minute : NaN;"
    yield "  } catch (_) {"
    yield "    return NaN;"
    yield "  }"
    yield "}"
    yield "function forecastMinuteInWindow(minutes, start, end){"
    yield "  return start <= end ? minutes >= start && minutes < end : minutes >= start || minutes < end;"
    yield "}"
    yield "function forecastSceneFor(data){"
    yield "  const rows = Array.isArray(data && data.hourly) ? data.hourly.slice(0, 3) : [];"
    yield "  if (!rows.length) return 'pending';"
    yield "  const symbols = rows.map((row) => String((row && row.symbol) || '').toLowerCase());"
    yield "  const symbolText = symbols.join(' ');"
    yield "  const precipitation = rows.reduce((total, row) => total + (Number(row && row.precip_mm) || 0), 0);"
    yield "  const cloudValues = rows.map((row) => Number(row && row.cloud)).filter(Number.isFinite);"
    yield "  const averageCloud = cloudValues.length ? cloudValues.reduce((total, value) => total + value, 0) / cloudValues.length : 0;"
    yield "  let condition = 'clear';"
    yield "  if (precipitation > 0.05 || /rain|shower|drizzle|thunder|sleet|snow/.test(symbolText)) condition = 'rain';"
    yield "  else if (averageCloud >= 45 || /cloud|overcast|fog|mist/.test(symbolText)) condition = 'cloudy';"
    yield "  const middle = rows[Math.min(1, rows.length - 1)] || {};"
    yield "  const middleSymbol = String(middle.symbol || '').toLowerCase();"
    yield "  const middleLocalMinutes = forecastClockMinutes(middle.local_time);"
    yield "  const fallbackMinutes = Number(middle.local_hour) * 60;"
    yield "  const stationMinutes = forecastStationMinutes(data);"
    yield "  const minutes = Number.isFinite(stationMinutes) ? stationMinutes : (Number.isFinite(middleLocalMinutes) ? middleLocalMinutes : fallbackMinutes);"
    yield "  const sunrise = forecastClockMinutes(astroData && astroData.sunrise);"
    yield "  const sunset = forecastClockMinutes(astroData && astroData.sunset);"
    yield "  let isNight;"
    yield "  if (Number.isFinite(sunrise) && Number.isFinite(sunset) && Number.isFinite(minutes)) {"
    yield "    const daybreak = (sunrise - 30 + 1440) % 1440;"
    yield "    const dusk = (sunset + 30) % 1440;"
    yield "    isNight = !forecastMinuteInWindow(minutes, daybreak, dusk);"
    yield "  } else if (/_night\\b|\\bnight\\b/.test(middleSymbol)) isNight = true;"
    yield "  else if (/_day\\b|\\bday\\b/.test(middleSymbol)) isNight = false;"
    yield "  else isNight = !(Number.isFinite(minutes) && minutes >= 7 * 60 && minutes < 19 * 60);"
    yield "  return `${condition}-${isNight ? 'night' : 'day'}`;"
    yield "}"
    yield "function applyForecastScene(data){"
    yield "  const card = document.getElementById('weatherForecastBox');"
    yield "  if (!card) return;"
    yield "  Array.from(card.classList).filter((name) => name.startsWith('forecast-scene-')).forEach((name) => card.classList.remove(name));"
    yield "  const scene = forecastSceneFor(data);"
    yield "  card.classList.add(`forecast-scene-${scene}`);"
    yield "  card.dataset.forecastScene = scene;"
    yield "}"
    yield "window.setInterval(function(){ if (window.__weatherForecastPayload) applyForecastScene(window.__weatherForecastPayload); }, 60000);"
    yield "function renderWeatherForecast(data){"
    yield "  window.__weatherForecastPayload = data || null;"
    yield "  applyForecastScene(data);"
    ...
    yield "    setForecastPrecipitation(data, null);"
    ...
    yield "  setForecastText('forecastTempRange', forecastReadingAndRange(forecastCurrentTemperature(data), cur.temp_range));"
    yield "  setForecastWind(cur.wind || '--');"
    yield "  setForecastText('forecastRhRange', forecastReadingAndRange(forecastCurrentHumidity(data), cur.rh_range));"
    yield "  setForecastPrecipitation(data, cur);"
    yield "  if (typeof bioRefreshOpenSummary === 'function') bioRefreshOpenSummary();"
    yield "}"
    ...
    yield "}"
    yield "window.openWeatherForecastModal = function(){"
    yield "  setForecastButtonLoading(true);"
    yield "  window.location.assign('/weather-forecast');"
    yield "};"
    yield ""
    ...
    yield "  const daylightEl = document.getElementById('bioDaylightLine');"
    yield "  const upcomingEl = document.getElementById('bioUpcoming');"
    yield "  if (!signEl || !elementEl || !openBtn || !boxEl || !panelEl || !dateEl || !windowEl || !daylightEl || !upcomingEl) return;"
    yield "  const titleEl = boxEl.querySelector('.astro-title');"
    yield "  const biodynamicWarming = isDashboardWarmingPayload(data);"
    yield "  if (typeof setBiodynamicCardLoading === 'function') setBiodynamicCardLoading(biodynamicWarming);"
    ...
    yield "  };"
    yield "  const findBiodynamicDay = (payload, currentIso) => {"
    yield "    const dayKey = isoDayKey(currentIso);"
    yield "    const days = Array.isArray(payload && payload.calendar) ? payload.calendar : [];"
    yield "    return days.find((d) => d && d.date === dayKey) || null;"
    yield "  };"
    yield "  const formatDaylightLabel = (minutes) => {"
    yield "    const total = parseInt(minutes, 10);"
    yield "    if (!Number.isFinite(total) || total < 0) return '';"
    yield "    return `${Math.floor(total / 60)} Hrs ${total % 60} Mins`;"
    yield "  };"
    yield "  const buildDaylightText = (payload, currentIso) => {"
    yield "    const day = findBiodynamicDay(payload, currentIso);"
    yield "    const label = String((day && day.daylight_label) || '').trim() || formatDaylightLabel(day && day.daylight_minutes);"
    yield "    return `Hours of Daylight: ${label || '--'}`;"
    yield "  };"
    ...
    yield "  };"
    yield "  const biodynamicActionColors = (item) => {"
    yield "    const isFruit = String((item && item.plant_part) || '').trim().toLowerCase() === 'fruit';"
    yield "    const background = isFruit ? '#4c3a7f' : String((item && item.color) || '').trim();"
    yield "    return { background, text: isFruit ? '#fff' : textOnHex(background) };"
    yield "  };"
    yield "  window.biodynamicActionColors = biodynamicActionColors;"
    ...
    yield "    if (reason === 'warming') return '';"
    ...
    yield "    signEl.textContent = biodynamicWarming ? 'Loading...' : 'Biodynamics unavailable';"
    yield "    elementEl.textContent = biodynamicWarming ? 'Moon sign' : 'Automatic ephemeris install is enabled.';"
    ...
    yield "    clearBiodynamicTextContrast([titleEl, signEl, elementEl, dateEl, windowEl, daylightEl, upcomingEl]);"
    yield "    dateEl.textContent = '';"
    yield "    windowEl.textContent = buildUnavailableMessage(data);"
    yield "    daylightEl.textContent = 'Hours of Daylight: --';"
    ...
    yield "  const actionColors = biodynamicActionColors(cur);"
    yield "  openBtn.style.background = actionColors.background;"
    yield "  openBtn.style.borderColor = '#27313a';"
    yield "  openBtn.style.color = actionColors.text;"
    ...
    yield "  applyBiodynamicTextContrast([titleEl, signEl, elementEl, dateEl, windowEl, daylightEl, upcomingEl], mainTextColor, mainTextGradient);"
    ...
    yield "  daylightEl.textContent = buildDaylightText(data, cur.timestamp);"
    yield "  upcomingEl.textContent = '';"
    yield "}"
    yield ""
    yield "window.__bioModalState = window.__bioModalState || { month: '', data: null, selectedDate: '' };"
    yield "window.__bioMonthCache = window.__bioMonthCache || {};"
    yield "window.__bioMonthInflight = window.__bioMonthInflight || {};"
    yield "window.__bioMonthRequestSeq = window.__bioMonthRequestSeq || 0;"
    yield "window.__bioSummaryRefresh = window.__bioSummaryRefresh || { date: '', attempts: 0, timer: null };"
    ...
    yield "function bioForecastSummaryText(){"
    yield "  const data = window.__weatherForecastPayload || null;"
    yield "  const cur = (data && data.current_24h && typeof data.current_24h === 'object') ? data.current_24h : null;"
    yield "  const lines = ['24hr Forecast'];"
    yield "  if (!data || !data.ok || !cur) {"
    yield "    lines.push(forecastUnavailableText(data));"
    yield "    return lines.join('\\n');"
    yield "  }"
    yield "  const overall = String(cur.overall || cur.forecast || '').trim();"
    yield "  const temp = String(cur.temp_range || '').trim();"
    yield "  const rh = String(cur.rh_range || '').trim();"
    yield "  const wind = String(cur.wind || '').trim().replace(/\\s*\\n\\s*/g, ' | ');"
    yield "  if (overall && overall !== '--') lines.push(`Overall: ${overall}`);"
    yield "  if (temp && temp !== '--') lines.push(`Temperature: ${temp}`);"
    yield "  if (rh && rh !== '--') lines.push(`Relative Humidity: ${rh}`);"
    yield "  if (wind && wind !== '--') lines.push(`Wind: ${wind}`);"
    yield "  if (Number.isFinite(Number(cur.precip_mm))) lines.push(`Precipitation: ${Number(cur.precip_mm).toFixed(2)} mm`);"
    yield "  return lines.join('\\n');"
    yield "}"
    yield "function bioSummaryWithForecast(dateIso, summaryText){"
    yield "  const parts = [];"
    ...
    yield "  if (!document.getElementById('biodynamicCalendarModal')) return;"
    yield "  const st = window.__bioModalState || {};"
    yield "  if (st.selectedDate && typeof bioSelectDate === 'function') bioSelectDate(st.selectedDate);"
    yield "}"
    ...
    yield "  const st = window.__bioModalState || {};"
    yield "  const data = st.data || {};"
    yield "  const reportEl = document.getElementById('bioPrintReportSheet');"
    yield "  if (!reportEl) return;"
    ...
    yield "    reportEl.innerHTML = `<div class='bio-print-title'>Biodynamic Calendar Report</div><div class='bio-print-subtitle'>No printable calendar data available.</div>`;"
    yield "    return;"
    yield "  }"
    ...
    yield "    return `<div class='${classes.join(' ')}' style='background:${dayBg};border-color:${bioEsc(day.dominant_color || '#d7d0bf')};'><div class='bio-print-day-head'><div class='bio-print-day-num'>${bioEsc(day.day)}</div></div></div>`;"
    yield "  }).join('');"
    ...
    yield "    const summaryText = bioSummaryWithForecast(day.date, summaries[day.date] || '');"
    ...
    yield "  reportEl.innerHTML = `<div class='bio-print-title'>Biodynamic Calendar Report</div><div class='bio-print-subtitle'>${bioEsc(data.month_label || '--')} | ${selectedMsg}</div><div class='bio-print-section-title'>Calendar</div><div class='bio-print-calendar'>${grid}</div><div class='bio-print-section-title'>Daily Summary and Notes</div><div class='bio-print-sections'>${entries}</div>`;"
    yield "}"
    yield "function bioRunPrint(){"
    yield "  renderBiodynamicPrintView();"
    yield "  document.body.classList.add('bio-printing');"
    yield "  window.print();"
    yield "};"
    yield "window.printBiodynamicReport = function(){ bioRunPrint(); };"
    yield "if (!window.__bioPrintCleanupBound) {"
    yield "  window.addEventListener('afterprint', () => document.body.classList.remove('bio-printing'));"
    ...
    yield "  const dateEl = document.getElementById('bioSelectedDate');"
    ...
    yield "  if (summaryEl) {"
    yield "    const storedSummary = dateIso ? String(summaries[dateIso] || '').trim() : '';"
    yield "    const summaryText = storedSummary ? bioSummaryWithForecast(dateIso, storedSummary) : '';"
    yield "    summaryEl.innerHTML = summaryText ? bioTextToHtml(summaryText) : '';"
    yield "    setBioSummaryLoading(!!dateIso && !storedSummary);"
    yield "    if (dateIso && !storedSummary) scheduleBioSummaryRefresh(dateIso);"
    yield "  }"
    yield "  if (statusEl) statusEl.textContent = '';"
    yield "  renderBiodynamicModalSummary();"
    yield "  renderBiodynamicPrintView();"
    yield "}"
    yield "function setBioSummaryLoading(isLoading){"
    yield "  const card = document.querySelector('#biodynamicCalendarModal .bio-summary-card');"
    yield "  if (card) { card.classList.toggle('summary-loading', !!isLoading); card.setAttribute('aria-busy', isLoading ? 'true' : 'false'); }"
    yield "}"
    yield "function scheduleBioSummaryRefresh(dateIso){"
    yield "  const refresh = window.__bioSummaryRefresh || (window.__bioSummaryRefresh = { date:'', attempts:0, timer:null });"
    yield "  if (refresh.date !== dateIso) { if (refresh.timer) clearTimeout(refresh.timer); refresh.date = dateIso; refresh.attempts = 0; refresh.timer = null; }"
    yield "  if (refresh.timer || refresh.attempts >= 40) return;"
    yield "  refresh.timer = setTimeout(async () => {"
    yield "    refresh.timer = null; refresh.attempts += 1;"
    yield "    if (!document.getElementById('biodynamicCalendarModal')) return;"
    yield "    const st = window.__bioModalState || {};"
    yield "    if (st.selectedDate !== dateIso) return;"
    yield "    try {"
    yield "      const data = await fetchBiodynamicMonth(st.month, true);"
    yield "      if (st.selectedDate !== dateIso) return;"
    yield "      st.data = data;"
    yield "      const summary = String(data?.daily_summaries?.[dateIso] || '').trim();"
    yield "      if (summary) { bioSelectDate(dateIso); return; }"
    yield "    } catch (_) {}"
    yield "    scheduleBioSummaryRefresh(dateIso);"
    yield "  }, 2000);"
    yield "}"
    ...
    yield "    return `<button type='button' class='${classes.join(' ')}' data-date='${bioEsc(day.date)}' style='${style}' title='${title}'><span class='bio-day-num'>${bioEsc(day.day)}</span></button>`;"
    yield "  }).join('');"
    ...
    yield "}"
    yield "function setBioMonthLoading(isLoading){"
    yield "  const modal = document.getElementById('biodynamicCalendarModal');"
    yield "  if (!modal) return;"
    yield "  modal.classList.toggle('loading-month', !!isLoading);"
    yield "  if (isLoading) setBioSummaryLoading(true);"
    yield "  const prev = document.getElementById('bioPrevMonthBtn');"
    yield "  const next = document.getElementById('bioNextMonthBtn');"
    yield "  [prev, next].forEach((btn) => { if (btn) { btn.disabled = !!isLoading; btn.setAttribute('aria-busy', isLoading ? 'true' : 'false'); } });"
    yield "}"
    yield "async function fetchBiodynamicMonth(monthKey, forceRefresh=false){"
    yield "  const key = String(monthKey || bioMonthKeyFromDate(new Date()));"
    yield "  window.__bioMonthCache = window.__bioMonthCache || {};"
    yield "  window.__bioMonthInflight = window.__bioMonthInflight || {};"
    yield "  if (!forceRefresh && window.__bioMonthCache[key]) return window.__bioMonthCache[key];"
    yield "  if (!forceRefresh && window.__bioMonthInflight[key]) return window.__bioMonthInflight[key];"
    yield "  window.__bioMonthInflight[key] = fetch(`/api/biodynamic-calendar?month=${encodeURIComponent(key)}`, { cache:'no-store' })"
    yield "    .then((resp) => { if (!resp.ok) throw new Error(`biodynamic_month_${resp.status}`); return resp.json(); })"
    yield "    .then((data) => { window.__bioMonthCache[key] = data; return data; })"
    yield "    .finally(() => { delete window.__bioMonthInflight[key]; });"
    yield "  return window.__bioMonthInflight[key];"
    yield "}"
    yield "function prefetchBiodynamicAdjacentMonths(monthKey){"
    yield "  const key = String(monthKey || '');"
    yield "  if (!key) return;"
    yield "  [-1, 1].forEach((delta) => {"
    yield "    const nextKey = bioShiftMonth(key, delta);"
    yield "    if (!nextKey || window.__bioMonthCache?.[nextKey] || window.__bioMonthInflight?.[nextKey]) return;"
    yield "    setTimeout(() => { fetchBiodynamicMonth(nextKey).catch(() => {}); }, 0);"
    yield "  });"
    yield "}"
    yield "async function loadBiodynamicMonth(monthKey, preferredDate){"
    yield "  const st = window.__bioModalState || {};"
    yield "  const key = String(monthKey || bioMonthKeyFromDate(new Date()));"
    yield "  const requestSeq = (window.__bioMonthRequestSeq || 0) + 1;"
    yield "  window.__bioMonthRequestSeq = requestSeq;"
    yield "  st.month = key;"
    yield "  setBioMonthLoading(true);"
    yield "  try {"
    yield "    const data = await fetchBiodynamicMonth(key);"
    yield "    if (requestSeq !== window.__bioMonthRequestSeq) return;"
    yield "    st.data = data;"
    yield "    st.selectedDate = preferredDate || st.selectedDate || '';"
    yield "    renderBiodynamicModal();"
    yield "    prefetchBiodynamicAdjacentMonths(key);"
    yield "  } finally {"
    yield "    if (requestSeq === window.__bioMonthRequestSeq) setBioMonthLoading(false);"
    yield "  }"
    yield "}"
    ...
    yield "  btn.setAttribute('aria-busy', isLoading ? 'true' : 'false');"
    yield "}"
    yield "window.openBiodynamicCalendar = function(){"
    yield "  setBioOpenButtonLoading(true);"
    yield "  window.requestAnimationFrame(function(){"
    yield "    window.requestAnimationFrame(function(){ window.location.assign('/calendar'); });"
    yield "  });"
    yield "};"
    ...
    yield "        <div class='bio-legend' aria-label='Biodynamic calendar color legend'>"
    yield "          <span class='bio-legend-item'><span class='bio-legend-swatch bio-legend-root' aria-hidden='true'></span>Root</span>"
    yield "          <span class='bio-legend-item'><span class='bio-legend-swatch bio-legend-leaf' aria-hidden='true'></span>Leaf</span>"
    yield "          <span class='bio-legend-item'><span class='bio-legend-swatch bio-legend-flower' aria-hidden='true'></span>Flower</span>"
    yield "          <span class='bio-legend-item'><span class='bio-legend-swatch bio-legend-fruit' aria-hidden='true'></span>Fruit</span>"
    yield "          <span class='bio-legend-item'><span class='bio-legend-swatch bio-legend-rest' aria-hidden='true'></span>Rest</span>"
    yield "          <span class='bio-legend-item'><span class='bio-legend-swatch bio-legend-transition' aria-hidden='true'></span>Transition</span>"
    yield "        </div>"
    yield "        <div class='bio-calendar-wrap'>"
    yield "          <div class='bio-calendar' id='bioModalCalendar'></div>"
    yield "          <div class='bio-loading-overlay bio-calendar-loading' role='status' aria-label='Loading biodynamic calendar'><span class='spinner' aria-hidden='true'></span></div>"
    yield "        </div>"
    yield "      </div>"
    yield "      <div class='bio-modal-side'>"
    yield "        <div class='bio-selected-date' id='bioSelectedDate'>Select a day</div>"
    ...
    yield "          <div class='bio-loading-overlay bio-summary-loading' role='status' aria-label='Loading daily summary'><span class='spinner' aria-hidden='true'></span></div>"
    yield "        </div>"
    yield "        <div class='bio-note-card bio-notes-card'>"
    yield "          <div class='bio-note-title'>Daily Notes</div>"
    ...
    yield "            <button type='button' class='bio-print-btn' id='bioPrintReportBtn'>Print Report</button>"
    yield "            <div class='bio-note-status' id='bioNoteStatus'></div>"
    yield "          </div>"
    yield "        </div>"
    yield "      </div>"
    yield "    </div>"
    yield "    <div class='bio-print-sheet' id='bioPrintReportSheet' aria-hidden='true'></div>"
    yield "  </div>"
    yield "</div>`;"
    ...
    yield "    document.getElementById('bioPrintReportBtn').addEventListener('click', () => { if (window.printBiodynamicReport) window.printBiodynamicReport(); });"
    ...
    yield "    forecastFiveDayBtn.addEventListener('click', function(){ window.openWeatherForecastModal(); });"
    yield "    window.addEventListener('pageshow', function(){ setForecastButtonLoading(false); });"
    yield "  }"
    yield "  if (bioOpenBtn) {"
    yield "    bioOpenBtn.addEventListener('click', function(){ if (window.openBiodynamicCalendar) window.openBiodynamicCalendar(); });"
    yield "    window.addEventListener('pageshow', function(){ setBioOpenButtonLoading(false); });"
    yield "  }"
    ...
    yield "    });"
    yield "  }"
    yield "  if (window.initializeSensorRowCollapse) window.initializeSensorRowCollapse();"
    yield "});"
    ...
    yield " const overviewFooter = document.getElementById('dashboard-overview-footer');"
    yield " const dashboard = document.querySelector('.dashboard');"
    yield " const parent = (overviewFooter && overviewFooter.parentElement) || (existingRow && existingRow.parentElement) || dashboard || byGraphModal || document.body;"
    ...
    yield "    else if (overviewFooter && overviewFooter.parentElement === parent) parent.insertBefore(group, overviewFooter);"
    ...
    yield "    header.className = 'sensor-group-title';"
    ...
    yield "    const lazyVisual = dashboardMetricSet !== 'All' && idx >= 6 && group.dataset.sensorExpanded !== 'true';"
    yield "    if (lazyVisual) container.dataset.lazyMetricVisual = '1';"
    yield ""
    yield "    const safe = `${sid}_${safeMetric}`;"
    yield "    const gaugeVisual = lazyVisual ? '' : `<div class='gauge-view' id='${safe}GaugeContainer'><canvas id='${safe}Gauge'></canvas></div>`;"
    yield "    const graphVisual = lazyVisual ? '' : `<div class='graph-view' id='${safe}GraphContainer'><canvas class='micrograph-canvas' id='${safe}Micrograph' width='${metricCanvasWidth}' height='${metricCanvasHeight}'></canvas></div>`;"
    yield "    container.innerHTML = "
    yield "      `<div class='metric-title'>${metric}</div>` +"
    yield "      `<div class='gauge-container'>${gaugeVisual}</div>` +"
    yield "      `<div class='graph-container'>${graphVisual}</div>` +"
    ...
    yield "    }"
    yield "  });"
    yield "  if (window.scheduleSensorRowCollapseRefresh) window.scheduleSensorRowCollapseRefresh(group);"
    yield "}"

    yield "window.ensureSensorCollapseToggle = function(group){"
    yield "  if (!group) return null;"
    yield "  const sid = String(group.dataset.sensorId || '').trim();"
    yield "  const header = group.querySelector('.sensor-group-title');"
    yield "  if (!sid || !header) return null;"
    yield "  let button = header.querySelector('.sensor-collapse-toggle');"
    yield "  if (!button) {"
    yield "    button = document.createElement('button');"
    yield "    button.type = 'button';"
    yield "    button.className = 'sensor-collapse-toggle';"
    yield "    button.dataset.sensorId = sid;"
    yield "    button.setAttribute('aria-controls', `row_${sid}`);"
    yield "    button.setAttribute('aria-expanded', 'false');"
    yield "    button.hidden = true;"
    ...
    yield "  return button;"
    yield "};"
    yield "window.refreshSensorRowCollapse = function(group){"
    yield "  if (!group || !document.body.contains(group)) return;"
    yield "  const sid = String(group.dataset.sensorId || '').trim();"
    ...
    yield "    return;"
    yield "  }"
    yield "  overflowCards.forEach((card) => card.classList.add('sensor-metric-overflow'));"
    yield "  const expanded = group.dataset.sensorExpanded === 'true';"
    yield "  const sidUpper = sid.toUpperCase();"
    yield "  row.classList.toggle('is-collapsed', !expanded);"
    yield "  if (expanded && typeof window.materializeExpandedMetricVisuals === 'function') {"
    yield "    window.materializeExpandedMetricVisuals(group);"
    yield "  } else if (!expanded && typeof window.releaseCollapsedMetricVisuals === 'function') {"
    yield "    window.releaseCollapsedMetricVisuals(group);"
    yield "  }"
    yield "  button.setAttribute('aria-expanded', expanded ? 'true' : 'false');"
    yield "  button.setAttribute('aria-label', `${expanded ? 'Collapse' : 'Expand'} ${sidUpper} metrics`);"
    yield "  button.title = `${expanded ? 'Collapse' : 'Expand'} ${sidUpper} metrics`;"
    yield "};"
    yield "window.scheduleSensorRowCollapseRefresh = function(group){"
    yield "  window.requestAnimationFrame(function(){"
    yield "    window.requestAnimationFrame(function(){ window.refreshSensorRowCollapse(group); });"
    yield "  });"
    yield "};"
    yield "window.initializeSensorRowCollapse = function(){"
    yield "  document.querySelectorAll('.sensor-group[data-sensor-id]').forEach((group) => {"
    yield "    window.scheduleSensorRowCollapseRefresh(group);"
    yield "  });"
    yield "};"
    yield "if (!window.__sensorRowCollapseInit) {"
    yield "  window.__sensorRowCollapseInit = true;"
    yield "  document.addEventListener('click', function(ev){"
    yield "    const button = ev.target.closest('.sensor-collapse-toggle');"
    yield "    if (!button) return;"
    yield "    const group = button.closest('.sensor-group[data-sensor-id]');"
    yield "    if (!group) return;"
    yield "    group.dataset.sensorExpanded = group.dataset.sensorExpanded === 'true' ? 'false' : 'true';"
    yield "    window.refreshSensorRowCollapse(group);"
    yield "  });"
    yield "  let collapseResizeTimer = 0;"
    ...
    yield "window.updateMetricCardTitle = function(container, style) {"
    yield "  if (!container) return;"
    yield "  const metric = String(container.dataset.metric || '').toLowerCase().replace(/[_-]+/g, ' ').replace(/\\s+/g, ' ').trim();"
    yield "  const title = container.querySelector('.metric-title');"
    yield "  if (!title) return;"
    yield "  const normalized = window.normalizeDisplayStyle ? window.normalizeDisplayStyle(style) : String(style || 'Gauge');"
    yield "  let baseTitle = String(container.dataset.metricCardTitle || title.textContent || '').replace(/^(?:6|24)hr\\s+/i, '');"
    yield "  if (!container.dataset.metricCardTitle) container.dataset.metricCardTitle = baseTitle;"
    yield "  if (metric === 'wind direction' && normalized !== 'Gauge') {"
    yield "    baseTitle = 'Wind-Rose';"
    yield "  } else if (metric === 'wind direction') {"
    yield "    const unit = String(gaugeConfig?.['Wind Direction']?.unit || 'deg');"
    yield "    baseTitle = `Wind Direction (${unit})`;"
    yield "  }"
    yield "  const durationPrefix = normalized === 'Graph6hr' ? '6hr ' : (normalized === 'Graph24hr' ? '24hr ' : '');"
    yield "  title.textContent = durationPrefix + baseTitle;"
    yield "};"
    ...
    yield "  if (typeof window.updateMetricCardTitle === 'function') window.updateMetricCardTitle(container, norm);"
    yield "};"
    ...
    yield "      if (window.registerContainerStyle) {"
    yield "        const existingStyle = window.normalizeDisplayStyle(container.dataset.displayStyle || window.displayStyle || 'Gauge');"
    yield "        window.registerContainerStyle(container, existingStyle);"
    yield "      }"
    yield "      return;"
    yield "    }"
    yield "    const opts = {"
    ...
    yield "        gauge.render();"
    yield "        gauge.__sensoriusCanvas = canvas;"
    ...
    yield "    }"    
    yield "    } catch (e) {"
    yield "      console.warn('initGauge: metric card init failed', container && container.id, e);"
    yield "    }"
    yield "  });"
    yield "}"
    ...
    yield "  const options = arguments.length ? arguments[0] : null;"
    yield "  const ignoreVisibility = !!(options && options.ignoreVisibility);"
    yield "  const ignoreModal = !!(options && options.ignoreModal);"
    ...
    yield "    || (!ignoreVisibility && document.visibilityState === 'hidden')"
    yield "    || (!ignoreModal && hasOpenBackdropModal());"
    yield "}"

    yield "function scheduleLayoutRefresh(reason, sig) {"
    yield "  if (pendingLayoutRefresh) return true;"
    yield "  if (hasOpenBackdropModal()) {"
    yield "    if (deferredLayoutRefresh) return true;"
    ...
    yield "    const doneSig = sessionStorage.getItem('layoutRefreshDoneSig') || '';"
    yield "    const prevAt = Number.parseInt(prevAtRaw, 10) || 0;"
    yield "    const nowMs = Date.now();"
    yield "    if (sig && doneSig === sig) {"
    yield "      const suppressedSig = sessionStorage.getItem('layoutRefreshSuppressedSig') || '';"
    yield "      if (suppressedSig !== sig) console.info('[layout-refresh-suppressed]', reason || 'layout changed');"
    yield "      sessionStorage.setItem('layoutRefreshSuppressedSig', sig);"
    yield "      return false;"
    yield "    }"
    yield "    if (sig && prevSig === sig && (nowMs - prevAt) < 20000) return false;"
    yield "    if (sig) sessionStorage.setItem('layoutRefreshSig', sig);"
    yield "    if (sig) sessionStorage.setItem('layoutRefreshDoneSig', sig);"
    yield "    sessionStorage.removeItem('layoutRefreshSuppressedSig');"
    ...
    yield "    const row = document.getElementById(`row_${sid}`);"
    yield "    if (!row) return { reason: `sensor:${sid}` };"
    ...
    yield "}"
    yield ""
    yield "window.__updateGaugesInFlight = window.__updateGaugesInFlight || false;"
    yield "window.__updateGaugesQueued = window.__updateGaugesQueued || false;"
    yield "window.__updateGaugesStartedAt = window.__updateGaugesStartedAt || 0;"
    yield "window.__updateGaugesRunSeq = window.__updateGaugesRunSeq || 0;"
    yield "window.__updateGaugesFinishedSeq = window.__updateGaugesFinishedSeq || 0;"
    yield "window.__updateGaugesFinishedAt = window.__updateGaugesFinishedAt || 0;"
    yield "window.__updateGaugesLastDurationMs = window.__updateGaugesLastDurationMs || 0;"
    yield "window.__updateGaugesLastOk = window.__updateGaugesLastOk || false;"
    yield "window.__updateGaugesLastError = window.__updateGaugesLastError || '';"
    yield "function finishUpdateGaugesRun(options, result) {"
    yield "  const finishedAt = Date.now();"
    yield "  const startedAt = window.__updateGaugesStartedAt || finishedAt;"
    yield "  window.__updateGaugesFinishedSeq = window.__updateGaugesRunSeq || window.__updateGaugesFinishedSeq || 0;"
    yield "  window.__updateGaugesFinishedAt = finishedAt;"
    yield "  window.__updateGaugesLastDurationMs = Math.max(0, finishedAt - startedAt);"
    yield "  window.__updateGaugesLastOk = !!(result && result.ok);"
    yield "  window.__updateGaugesLastError = String((result && result.error) || '');"
    yield "  window.__updateGaugesInFlight = false;"
    yield "  window.__updateGaugesStartedAt = 0;"
    yield "  if (window.__updateGaugesQueued) {"
    yield "    window.__updateGaugesQueued = false;"
    yield "    setTimeout(() => updateGauges(options || {}), 0);"
    yield "  }"
    yield "}"
    ...
    yield "  const ignoreVisibility = !!(options && options.ignoreVisibility);"
    yield "  const ignoreModal = !!(options && options.ignoreModal);"
    yield "  if (typeof dashboardRefreshPaused === 'function' && dashboardRefreshPaused({ ignoreVisibility, ignoreModal })) {"
    yield "    return;"
    yield "  }"
    yield "  const nowForInflight = Date.now();"
    yield "  if (window.__updateGaugesInFlight && (nowForInflight - window.__updateGaugesStartedAt) < (dashboardFetchTimeoutMs + 3000)) {"
    yield "    window.__updateGaugesQueued = true;"
    yield "    return;"
    yield "  }"
    yield "  window.__updateGaugesInFlight = true;"
    yield "  window.__updateGaugesStartedAt = nowForInflight;"
    yield "  window.__updateGaugesRunSeq = (window.__updateGaugesRunSeq || 0) + 1;"
    yield "  window.__updateGaugesLastOk = false;"
    yield "  window.__updateGaugesLastError = '';"
    yield "  const finishOptions = { ignoreVisibility, ignoreModal };"
    ...
    yield "  try {"
    yield "    const res = await fetchWithTimeout(window.location.pathname + '?json_only=true&sensor_id=' + encodeURIComponent(sensorId), { cache: 'no-store' }, dashboardFetchTimeoutMs);"
    yield "    if (!res.ok) {"
    yield "      console.warn('updateGauges: non-OK response', res.status);"
    yield "      finishUpdateGaugesRun(finishOptions, { ok: false, error: `http_${res.status}` });"
    yield "      return;"
    yield "    }"
    ...
    yield "    try {"
    yield "      if (typeof applySensorStatuses === 'function') applySensorStatuses(d);"
    yield "    } catch (e) {"
    yield "      console.warn('updateGauges: status update failed', e);"
    yield "    }"
    ...
    yield "        }"
    yield "      } catch (e) {"
    yield "        console.warn('updateGauges: switch status scheduling failed', e);"
    yield "      }"
    yield "    }"
    yield "  } catch (e) {"
    yield "    console.warn('updateGauges: fetch failed', e);"
    yield "    finishUpdateGaugesRun(finishOptions, { ok: false, error: String(e) });"
    yield "    return;"
    yield "  }"
    yield ""
    ...
    yield "  let layoutDrift = null;"
    yield "  try {"
    yield "    layoutDrift = shouldRefreshForLayoutDrift(available, nextExpMap, renderableSwitches, sensorId);"
    yield "  } catch (e) {"
    yield "    console.warn('updateGauges: layout drift check failed', e);"
    yield "  }"
    ...
    yield "      }"
    yield "      if (scheduleLayoutRefresh(reason, sig)) { finishUpdateGaugesRun(finishOptions, { ok: true }); return; }"
    yield "    } else {"
    yield "      if (scheduleLayoutRefresh(layoutDrift.reason, sig)) { finishUpdateGaugesRun(finishOptions, { ok: true }); return; }"
    yield "    }"
    yield "  } else {"
    yield "    try { sessionStorage.removeItem('layoutRefreshDoneSig'); sessionStorage.removeItem('layoutRefreshSuppressedSig'); } catch (_) {}"
    yield "  }"
    yield ""
    yield ""
    ...
    yield "    }"
    yield "    } catch (e) {"
    yield "      console.warn('updateGauges: sensor UI sync failed', sid, e);"
    yield "    }"
    ...
    yield "        } catch (e) {"
    yield "          console.warn('updateGauges: metric update failed', sid, metric, e);"
    yield "        }"
    yield "      }"
    yield "    } catch (e) {"
    yield "      console.warn('updateGauges: sensor update failed', sid, e);"
    yield "    }"
    yield "  }"
    yield ""
    yield "  const ts = document.getElementById('update_time');"
    ...
    yield ""
    yield "}"
    yield "window.updateGauges = updateGauges;"
    ...
    yield "window.__lastMicrographForceRefreshAt = window.__lastMicrographForceRefreshAt || 0;"
    yield "const micrographCacheMaxEntries = 24;"
    yield "const micrographCacheMaxBytes = 8 * 1024 * 1024;"
    yield "const micrographCacheMaxAgeMs = 5 * 60 * 1000;"
    yield "function trimMicrographDataCache(now = Date.now()) {"
    yield "  const cache = window.__micrographDataCache;"
    yield "  if (!(cache instanceof Map)) return;"
    yield "  for (const [key, entry] of cache) {"
    yield "    if (!entry || (now - Number(entry.at || 0)) > micrographCacheMaxAgeMs) cache.delete(key);"
    yield "  }"
    yield "  let totalBytes = 0;"
    yield "  for (const entry of cache.values()) totalBytes += Number(entry && entry.bytes || 0);"
    yield "  while (cache.size > micrographCacheMaxEntries || totalBytes > micrographCacheMaxBytes) {"
    yield "    const oldestKey = cache.keys().next().value;"
    yield "    if (oldestKey === undefined) break;"
    yield "    const oldest = cache.get(oldestKey);"
    yield "    totalBytes -= Number(oldest && oldest.bytes || 0);"
    yield "    cache.delete(oldestKey);"
    yield "  }"
    yield "}"
    yield "async function getMicrographJson(requestKey, url, force) {"
    yield "  const now = Date.now();"
    yield "  const cacheTtlMs = 60000;"
    yield "  trimMicrographDataCache(now);"
    yield "  const cached = window.__micrographDataCache.get(requestKey);"
    yield "  if (!force && cached && (now - cached.at) < cacheTtlMs) {"
    yield "    window.__micrographDataCache.delete(requestKey);"
    yield "    window.__micrographDataCache.set(requestKey, cached);"
    yield "    return cached.data;"
    yield "  }"
    yield "  const existing = window.__micrographDataInflight.get(requestKey);"
    yield "  if (existing) {"
    yield "    const existingPromise = existing.promise || existing;"
    yield "    const existingAt = Number(existing.at || 0);"
    yield "    if (!force && (!existingAt || (now - existingAt) < micrographInflightStaleMs)) {"
    yield "      return existingPromise;"
    yield "    }"
    yield "    window.__micrographDataInflight.delete(requestKey);"
    yield "  }"
    yield "  const promise = fetchWithTimeout(url, { cache: 'no-store' }, micrographFetchTimeoutMs).then(async (resp) => {"
    yield "    if (!resp.ok) {"
    yield "      throw new Error('micrograph_http_' + resp.status);"
    yield "    }"
    yield "    const data = await resp.json();"
    yield "    const contentLength = Number(resp.headers.get('content-length') || 0);"
    yield "    const bytes = contentLength > 0 ? contentLength : JSON.stringify(data).length;"
    yield "    window.__micrographDataCache.delete(requestKey);"
    yield "    window.__micrographDataCache.set(requestKey, { at: Date.now(), bytes, data });"
    yield "    trimMicrographDataCache();"
    yield "    return data;"
    yield "  }).finally(() => {"
    yield "    window.__micrographDataInflight.delete(requestKey);"
    yield "  });"
    yield "  window.__micrographDataInflight.set(requestKey, { promise, at: now });"
    yield "  return promise;"
    yield "}"
    yield ""
    yield "function renderWindRoseMicrograph(canvas, directionSeries, speedSeries, rangeLabel) {"
    yield "  if (!canvas || !directionSeries || !speedSeries) return false;"
    yield "  const directionTs = Array.isArray(directionSeries.ts) ? directionSeries.ts : [];"
    yield "  const directions = Array.isArray(directionSeries.vals) ? directionSeries.vals : [];"
    yield "  const speedTs = Array.isArray(speedSeries.ts) ? speedSeries.ts : [];"
    yield "  const speeds = Array.isArray(speedSeries.vals) ? speedSeries.vals : [];"
    yield "  const speedByTimestamp = new Map();"
    yield "  speedTs.forEach((ts, index) => {"
    yield "    const speed = Number(speeds[index]);"
    yield "    if (Number.isFinite(speed)) speedByTimestamp.set(String(ts), speed);"
    yield "  });"
    yield "  const baseSpeedBands = ["
    yield "    { min: 0, max: 5, label: '0-5', color: '#75bce8' },"
    yield "    { min: 5, max: 15, label: '5-15', color: '#5290bf' },"
    yield "    { min: 15, max: 30, label: '15-30', color: '#2e6396' },"
    yield "    { min: 30, max: Infinity, label: '30+', color: '#0b376d' }"
    yield "  ];"
    yield "  const speedConfig = gaugeConfig?.['Wind Speed'] || {};"
    yield "  const speedUnit = String(speedConfig.unit || speedConfig.source_unit || 'mph').trim();"
    yield "  const convertSpeedBandLimit = value => Number.isFinite(value)"
    yield "    ? Math.ceil(convertForDisplay(value, speedConfig) ?? value)"
    yield "    : Infinity;"
    yield "  const speedBands = baseSpeedBands.map(band => {"
    yield "    const min = convertSpeedBandLimit(band.min);"
    yield "    const max = convertSpeedBandLimit(band.max);"
    yield "    return { ...band, min, max, label: Number.isFinite(max) ? `${min}-${max}` : `${min}+` };"
    yield "  });"
    yield "  const directionNames = ['N','NNE','NE','ENE','E','ESE','SE','SSE','S','SSW','SW','WSW','W','WNW','NW','NNW'];"
    yield "  const bins = directionNames.map(() => speedBands.map(() => 0));"
    yield "  let sampleCount = 0;"
    yield "  let speedTotal = 0;"
    yield "  directions.forEach((rawDirection, index) => {"
    yield "    const direction = Number(rawDirection);"
    yield "    let rawSpeed = speedByTimestamp.get(String(directionTs[index]));"
    yield "    if (!Number.isFinite(rawSpeed) && directions.length === speeds.length) rawSpeed = Number(speeds[index]);"
    yield "    const speed = convertForDisplay(rawSpeed, speedConfig);"
    yield "    if (!Number.isFinite(direction) || !Number.isFinite(speed) || speed < 0) return;"
    yield "    const normalized = ((direction % 360) + 360) % 360;"
    yield "    const directionIndex = Math.floor((normalized + 11.25) / 22.5) % 16;"
    yield "    const speedIndex = speedBands.findIndex(band => speed >= band.min && speed < band.max);"
    yield "    if (speedIndex < 0) return;"
    yield "    bins[directionIndex][speedIndex] += 1;"
    yield "    sampleCount += 1;"
    yield "    speedTotal += speed;"
    yield "  });"
    yield "  if (!sampleCount) return false;"
    yield "  const oldChart = chartMap.get(canvas);"
    yield "  if (oldChart && typeof oldChart.destroy === 'function') oldChart.destroy();"
    yield "  chartMap.delete(canvas);"
    yield "  const ctx = canvas.getContext('2d');"
    yield "  const width = Number(canvas.width) || 260;"
    yield "  const height = Number(canvas.height) || 205;"
    yield "  ctx.clearRect(0, 0, width, height);"
    yield "  const style = window.getComputedStyle ? window.getComputedStyle(canvas) : null;"
    yield "  const textColor = (style && style.color) || '#f2f6fb';"
    yield "  const centerX = width / 2;"
    yield "  const legendHeight = 30;"
    yield "  const centerY = (height - legendHeight) / 2 + 2;"
    yield "  const radius = Math.max(34, Math.min(width / 2 - 25, (height - legendHeight) / 2 - 16));"
    yield "  const innerRadius = Math.max(13, radius * 0.23);"
    yield "  const directionTotals = bins.map(parts => parts.reduce((sum, count) => sum + count, 0));"
    yield "  const maxDirectionCount = Math.max(1, ...directionTotals);"
    yield "  ctx.save();"
    yield "  ctx.strokeStyle = 'rgba(130,160,195,0.28)';"
    yield "  ctx.lineWidth = 1;"
    yield "  [0.25, 0.5, 0.75, 1].forEach(fraction => {"
    yield "    ctx.beginPath();"
    yield "    ctx.arc(centerX, centerY, innerRadius + (radius - innerRadius) * fraction, 0, Math.PI * 2);"
    yield "    ctx.stroke();"
    yield "  });"
    yield "  const sectorHalfWidth = (Math.PI * 2 / 16) * 0.39;"
    yield "  bins.forEach((parts, directionIndex) => {"
    yield "    const centerAngle = directionIndex * Math.PI * 2 / 16 - Math.PI / 2;"
    yield "    let cumulative = 0;"
    yield "    parts.forEach((count, speedIndex) => {"
    yield "      if (!count) return;"
    yield "      const previous = cumulative;"
    yield "      cumulative += count;"
    yield "      const inner = innerRadius + (radius - innerRadius) * previous / maxDirectionCount;"
    yield "      const outer = innerRadius + (radius - innerRadius) * cumulative / maxDirectionCount;"
    yield "      const startAngle = centerAngle - sectorHalfWidth;"
    yield "      const endAngle = centerAngle + sectorHalfWidth;"
    yield "      ctx.beginPath();"
    yield "      ctx.arc(centerX, centerY, outer, startAngle, endAngle);"
    yield "      ctx.arc(centerX, centerY, inner, endAngle, startAngle, true);"
    yield "      ctx.closePath();"
    yield "      ctx.fillStyle = speedBands[speedIndex].color;"
    yield "      ctx.fill();"
    yield "    });"
    yield "  });"
    yield "  const cardinals = ["
    yield "    ['N', centerX, centerY - radius - 8],"
    yield "    ['E', centerX + radius + 10, centerY + 3],"
    yield "    ['S', centerX, centerY + radius + 12],"
    yield "    ['W', centerX - radius - 10, centerY + 3]"
    yield "  ];"
    yield "  ctx.fillStyle = textColor;"
    yield "  ctx.font = '600 11px system-ui, sans-serif';"
    yield "  ctx.textAlign = 'center';"
    yield "  cardinals.forEach(item => ctx.fillText(item[0], item[1], item[2]));"
    yield "  const dominantIndex = directionTotals.indexOf(Math.max(...directionTotals));"
    yield "  const dominantPercent = Math.round(directionTotals[dominantIndex] * 100 / sampleCount);"
    yield "  ctx.beginPath();"
    yield "  ctx.arc(centerX, centerY, innerRadius - 1, 0, Math.PI * 2);"
    yield "  ctx.fillStyle = 'rgba(15,23,36,0.92)';"
    yield "  ctx.fill();"
    yield "  ctx.fillStyle = '#f2f6fb';"
    yield "  ctx.font = '600 10px system-ui, sans-serif';"
    yield "  ctx.fillText(directionNames[dominantIndex], centerX, centerY - 1);"
    yield "  ctx.font = '9px system-ui, sans-serif';"
    yield "  ctx.fillText(String(dominantPercent) + '%', centerX, centerY + 9);"
    yield "  const legendY = height - 10;"
    yield "  const legendItemWidth = Math.min(58, (width - 12) / speedBands.length);"
    yield "  const legendStartX = centerX - legendItemWidth * speedBands.length / 2;"
    yield "  ctx.font = '9px system-ui, sans-serif';"
    yield "  ctx.textAlign = 'left';"
    yield "  speedBands.forEach((band, index) => {"
    yield "    const x = legendStartX + index * legendItemWidth;"
    yield "    ctx.fillStyle = band.color;"
    yield "    ctx.fillRect(x, legendY - 8, 9, 9);"
    yield "    ctx.fillStyle = textColor;"
    yield "    ctx.fillText(band.label, x + 12, legendY);"
    yield "  });"
    yield "  ctx.textAlign = 'right';"
    yield "  ctx.fillText(speedUnit, width - 3, legendY);"
    yield "  ctx.restore();"
    yield "  const averageSpeed = speedTotal / sampleCount;"
    yield "  canvas.setAttribute('role', 'img');"
    yield "  canvas.setAttribute('aria-label', `${rangeLabel} wind rose. Dominant direction ${directionNames[dominantIndex]} at ${dominantPercent} percent. Average wind speed ${averageSpeed.toFixed(1)} ${speedUnit}.`);"
    yield "  canvas.title = `${rangeLabel} wind rose — ${sampleCount} samples — dominant ${directionNames[dominantIndex]} ${dominantPercent}% — average ${averageSpeed.toFixed(1)} ${speedUnit}`;"
    yield "  return true;"
    yield "}"
    yield "window.renderWindRoseMicrograph = renderWindRoseMicrograph;"
    yield ""
    yield "async function showMicrographForContainer(container, options = {}) {"
    yield "  if (!container) return;"
    yield "  const forceFetch = !!(options && options.force);"
    yield "  const ignoreModal = !!(options && options.ignoreModal);"
    yield "  if (!forceFetch && typeof dashboardRefreshPaused === 'function' && dashboardRefreshPaused({ ignoreModal })) {"
    yield "    return;"
    yield "  }"
    ...
    yield "    if (!forceFetch && canvas.dataset.micrographInflight === requestKey) {"
    yield "      const inflightAt = Number.parseInt(canvas.dataset.micrographInflightAt || '0', 10) || 0;"
    yield "      if (!inflightAt || (Date.now() - inflightAt) < micrographInflightStaleMs) {"
    yield "      canvas.style.cursor = 'default';"
    yield "      return;"
    yield "      }"
    yield "      canvas.dataset.micrographInflight = '';"
    yield "      canvas.dataset.micrographInflightAt = '';"
    ...
    yield "    canvas.dataset.micrographInflightAt = String(Date.now());"
    yield "    container.dataset.micrographLoading = '1';"
    yield "    canvas.style.cursor = 'wait';"
    yield ""
    yield "    const metricNormForRequest = String(metric || '').toLowerCase().replace(/[_-]+/g, ' ').replace(/\\s+/g, ' ').trim();"
    yield "    const isWindDirectionRequest = metricNormForRequest === 'wind direction';"
    yield "    let url = '/graph-data?sensor_id=' + encodeURIComponent(sensor)"
    yield "              + '&metric1=' + encodeURIComponent(metric)"
    yield "              + '&range=' + encodeURIComponent(range);"
    yield "    if (isWindDirectionRequest) {"
    yield "      url += '&sensor_id2=' + encodeURIComponent(sensor) + '&metric2=' + encodeURIComponent('Wind Speed');"
    yield "    }"
    ...
    yield "    const isWindDirection = metricNorm.replace(/\\s+/g, ' ').trim() === 'wind direction';"
    yield "    if (isWindDirection) {"
    yield "      const speedKey = sensor + '::Wind Speed';"
    yield "      let speedSeries = allSeries[speedKey] || null;"
    yield "      if (!speedSeries) {"
    yield "        const speedEntry = entries.find(kv => String(kv[0] || '').toLowerCase().endsWith('::wind speed'));"
    yield "        speedSeries = speedEntry ? speedEntry[1] : null;"
    yield "      }"
    yield "      if (renderWindRoseMicrograph(canvas, seriesObj, speedSeries, xTitleText)) {"
    yield "        graph.style.display = 'block';"
    yield "        gauge.style.display = 'none';"
    yield "        return;"
    yield "      }"
    ...
    yield "      const date = new Date(val);"
    yield "      if (!Number.isFinite(date.getTime())) return '';"
    yield "      const pad2 = (part) => String(part).padStart(2, '0');"
    yield "      if (date.getHours() === 0 && date.getMinutes() === 0) {"
    ...
    yield "window.materializeMetricVisual = function(container) {"
    yield "  if (!container || container.dataset.lazyMetricVisual !== '1') return false;"
    yield "  const sensor = String(container.dataset.sensor || '').trim();"
    yield "  const metric = String(container.dataset.metric || '').trim();"
    yield "  if (!sensor || !metric) return false;"
    yield "  const safe = `${sensor}_${toSafe(metric)}`;"
    yield "  const gauge = container.querySelector('.gauge-container');"
    yield "  const graph = container.querySelector('.graph-container');"
    yield "  if (!gauge || !graph) return false;"
    yield "  gauge.innerHTML = `<div class='gauge-view' id='${safe}GaugeContainer'><canvas id='${safe}Gauge'></canvas></div>`;"
    yield "  graph.innerHTML = `<div class='graph-view' id='${safe}GraphContainer'><canvas class='micrograph-canvas' id='${safe}Micrograph' width='${metricCanvasWidth}' height='${metricCanvasHeight}'></canvas></div>`;"
    yield "  delete container.dataset.lazyMetricVisual;"
    yield "  if (typeof window.bindMetricContainer === 'function') window.bindMetricContainer(container);"
    yield "  return true;"
    yield "};"
    yield "window.materializeExpandedMetricVisuals = function(group) {"
    yield "  if (!group) return;"
    yield "  const cards = Array.from(group.querySelectorAll('.metric-container.sensor-metric-overflow'));"
    yield "  let changed = false;"
    yield "  cards.forEach((card) => { if (window.materializeMetricVisual(card)) changed = true; });"
    yield "  if (!changed) return;"
    yield "  if (typeof initGauge === 'function') initGauge();"
    yield "  window.setTimeout(function() {"
    yield "    cards.forEach((card) => {"
    yield "      const style = typeof window.getContainerStyle === 'function' ? window.getContainerStyle(card) : 'Gauge';"
    yield "      if ((style === 'Graph6hr' || style === 'Graph24hr') && typeof showMicrographForContainer === 'function') {"
    yield "        showMicrographForContainer(card, { force: false, ignoreModal: true });"
    yield "      }"
    yield "    });"
    yield "  }, 0);"
    yield "};"
    yield "window.releaseCollapsedMetricVisuals = function(group) {"
    yield "  if (!group) return;"
    yield "  group.querySelectorAll('.metric-container.sensor-metric-overflow').forEach((container) => {"
    yield "    const sensor = String(container.dataset.sensor || '').trim();"
    yield "    const metric = String(container.dataset.metric || '').trim();"
    yield "    const safe = `${sensor}_${toSafe(metric)}`;"
    yield "    const graphCanvas = container.querySelector('.micrograph-canvas');"
    yield "    const chart = graphCanvas ? chartMap.get(graphCanvas) : null;"
    yield "    if (chart && typeof chart.destroy === 'function') chart.destroy();"
    yield "    if (graphCanvas) chartMap.delete(graphCanvas);"
    yield "    const gaugeObject = window[`${safe}_gauge`];"
    yield "    if (gaugeObject && gaugeObject.__sensoriusCanvas && container.contains(gaugeObject.__sensoriusCanvas)) {"
    yield "      delete window[`${safe}_gauge`];"
    yield "    }"
    yield "    delete window[`${safe}_compass`];"
    yield "    const gauge = container.querySelector('.gauge-container');"
    yield "    const graph = container.querySelector('.graph-container');"
    yield "    if (gauge) gauge.innerHTML = '';"
    yield "    if (graph) graph.innerHTML = '';"
    yield "    container.dataset.lazyMetricVisual = '1';"
    yield "    window.__micrographDataCache.delete(`${sensor}::${metric}::6h`);"
    yield "    window.__micrographDataCache.delete(`${sensor}::${metric}::24h`);"
    yield "  });"
    yield "};"
    ...
    yield "    if (!canvas || !gauge || !graph) { return; }"
    yield "    const now = Date.now();"
    yield "    const lastClick = Number.parseInt(container.dataset.metricClickAt || '0', 10) || 0;"
    yield "    if ((now - lastClick) < 250) { return; }"
    yield "    container.dataset.metricClickAt = String(now);"
    ...
    yield "    graph.style.display = 'block';"
    yield "    gauge.style.display = 'none';"
    yield "    showMicrographForContainer(container, { force: true });"
    yield "};"
    yield ""
    ...
    yield ""
    yield "  async function refreshAllMicrographs(force = false, options = {}) {"
    yield "    const now = Date.now();"
    yield "    const ignoreModal = !!(options && options.ignoreModal);"
    yield "    if (typeof dashboardRefreshPaused === 'function' && dashboardRefreshPaused({ ignoreVisibility: !!force, ignoreModal })) {"
    yield "      return;"
    yield "    }"
    yield "    if (force && window.__lastMicrographForceRefreshAt && (now - window.__lastMicrographForceRefreshAt) < MIN_FORCE_INTERVAL_MS) {"
    yield "      return;"
    yield "    }"
    ...
    yield "    if (force) window.__lastMicrographForceRefreshAt = now;"
    yield ""
    ...
    yield "          await showMicrographForContainer(container, { force, ignoreModal });"
    yield "        }"
    yield "      } catch (e) {"
    ...
    yield "  });"
    yield ""
    yield "  document.addEventListener('visibilitychange', function() {"
    yield "    if (document.visibilityState === 'visible') {"
    yield "      setTimeout(() => updateGauges({ ignoreVisibility: true, ignoreModal: true }), 50);"
    yield "      setTimeout(() => refreshAllMicrographs(true, { ignoreModal: true }), 800);"
    yield "    }"
    yield "  });"
    yield "  window.addEventListener('focus', function() {"
    yield "    setTimeout(() => updateGauges({ ignoreVisibility: true, ignoreModal: true }), 50);"
    yield "    setTimeout(() => refreshAllMicrographs(true, { ignoreModal: true }), 800);"
    yield "  });"
    yield "})();"
    ...
    yield "    if (depth <= 0) startedAt = Date.now();"
    yield "    depth += 1;"
    yield "    apply();"
    yield "  }"
    yield "  function end(){"
    yield "    depth = Math.max(0, depth - 1);"
    yield "    if (depth === 0) startedAt = 0;"
    yield "    apply();"
    yield "  }"
    yield "  function isBusy(){"
    yield "    if (depth > 0 && startedAt && (Date.now() - startedAt) > 30000) {"
    yield "      depth = 0;"
    yield "      startedAt = 0;"
    yield "      apply();"
    yield "      return false;"
    ...
    # --- General Settings modal opener (embed into dashboard, avoid full-page nav) ---
    ...
    yield "      alert('Failed to load General Settings');"
    yield "      return;"
    yield "    }"
    yield "    if (!html || !html.trim()) {"
    ...
    yield "if (typeof window.getToastContainer !== 'function') {"
    yield "  window.getToastContainer = function(context){"
    yield "    function visible(el){"
    yield "      if (!el || !document.body.contains(el)) return false;"
    yield "      const style = window.getComputedStyle(el);"
    yield "      if (style.display === 'none' || style.visibility === 'hidden') return false;"
    yield "      const rect = el.getBoundingClientRect();"
    yield "      return rect.width > 0 && rect.height > 0;"
    yield "    }"
    yield "    const paneSelectors = ['.system-settings-content', '.switch-dialog-content', '.modal-body .split > section.panel'];"
    yield "    function paneFrom(root){"
    yield "      if (!root || !document.body.contains(root)) return null;"
    yield "      const selectors = paneSelectors;"
    yield "      for (const selector of selectors) {"
    yield "        if (root.matches && root.matches(selector) && visible(root)) return root;"
    yield "        const pane = root.querySelector(selector);"
    yield "        if (visible(pane)) return pane;"
    yield "      }"
    yield "      return null;"
    yield "    }"
    yield "    function contextRoot(){"
    yield "      if (!context) return null;"
    yield "      const el = context.nodeType === 1 ? context : context.parentElement;"
    yield "      if (!el || !document.body.contains(el)) return null;"
    yield "      return el.closest('.modal-backdrop') || el.closest('.system-settings-shell, .modal') || el;"
    yield "    }"
    yield "    function activePane(){"
    yield "      const contextual = paneFrom(contextRoot());"
    yield "      if (contextual) return contextual;"
    yield "      const roots = Array.from(document.querySelectorAll('.modal-backdrop, .system-settings-shell, .modal')).filter(visible);"
    yield "      for (let i = roots.length - 1; i >= 0; i -= 1) {"
    yield "        const pane = paneFrom(roots[i]);"
    yield "        if (pane) return pane;"
    yield "      }"
    yield "      const panes = Array.from(document.querySelectorAll(paneSelectors.join(','))).filter(visible);"
    yield "      if (panes.length) return panes[panes.length - 1];"
    yield "      return null;"
    yield "    }"
    yield "    const pane = activePane();"
    yield "    if (pane) {"
    yield "      let c = Array.from(pane.children).find((el) => el.classList && el.classList.contains('toast-container') && el.classList.contains('modal-pane-toast-container'));"
    yield "      if (!c) { c = document.createElement('div'); c.className = 'toast-container modal-pane-toast-container'; pane.appendChild(c); }"
    yield "      return c;"
    yield "    }"
    yield "    let c = document.querySelector('body > .toast-container:not(.modal-pane-toast-container)');"
    ...
    yield "  window.showToast = function(text, type, context){"
    yield "    const c = window.getToastContainer ? window.getToastContainer(context) : null;"
    yield "    if (!c) return;"
    ...
    yield "  });"
    yield "};"
    yield "window.pinDashboardOverviewFooter = function(){"
    yield "  const overviewFooter = document.getElementById('dashboard-overview-footer');"
    yield "  if (!overviewFooter) return;"
    yield "  const sensorGroup = document.querySelector('.sensor-group[data-sensor-id]');"
    yield "  const dashboardContent = (sensorGroup && sensorGroup.parentElement) || overviewFooter.parentElement || document.querySelector('.dashboard-content');"
    yield "  if (!dashboardContent) return;"
    yield "  if (overviewFooter.parentElement !== dashboardContent || dashboardContent.lastElementChild !== overviewFooter) dashboardContent.appendChild(overviewFooter);"
    yield "};"
    yield "window.applySensorGroupOrder = function(order){"
    yield "  if (!Array.isArray(order) || !order.length) return;"
    yield "  const overviewFooter = document.getElementById('dashboard-overview-footer');"
    yield "  const firstSensorGroup = document.querySelector('.sensor-group[data-sensor-id]');"
    yield "  const dashboardContent = (firstSensorGroup && firstSensorGroup.parentElement) || (overviewFooter && overviewFooter.parentElement) || document.querySelector('.dashboard-content');"
    yield "  if (!dashboardContent) return;"
    yield "  const insertBeforeFooter = (el) => {"
    yield "    if (!el) return;"
    yield "    if (overviewFooter && overviewFooter.parentElement === dashboardContent) dashboardContent.insertBefore(el, overviewFooter);"
    yield "    else dashboardContent.appendChild(el);"
    yield "  };"
    yield "  const groups = new Map();"
    yield "  dashboardContent.querySelectorAll('.sensor-group[data-sensor-id]').forEach((el) => {"
    yield "    groups.set(el.getAttribute('data-sensor-id') || '', el);"
    yield "  });"
    yield "  order.forEach((sid) => {"
    yield "    const el = groups.get(String(sid || ''));"
    yield "    if (el) insertBeforeFooter(el);"
    yield "  });"
    yield "  const switchGroup = document.getElementById('group_switches');"
    yield "  if (switchGroup) insertBeforeFooter(switchGroup);"
    yield "  window.pinDashboardOverviewFooter();"
    yield "};"
    ...
    # --- Switch Settings modal section switching ---
    ...
    yield "  const paneStats = modal.querySelector('#switchStatisticsPane');"
    ...
    yield "  const locationInput = form ? form.querySelector('input[name=\"location\"]') : null;"
    yield "  const initialLocation = locationInput ? String(locationInput.value || '').trim() : '';"
    yield "  if (!btnSettings || !btnStats || !paneSettings || !paneStats) return;"
    yield ""
    yield "  function formatStatDuration(seconds){"
    yield "    if (seconds === null || seconds === undefined || !Number.isFinite(seconds)) return 'No offline events';"
    yield "    let total = Math.max(0, Math.floor(seconds));"
    yield "    const days = Math.floor(total / 86400);"
    yield "    total -= days * 86400;"
    yield "    const hours = Math.floor(total / 3600);"
    yield "    total -= hours * 3600;"
    yield "    const minutes = Math.floor(total / 60);"
    yield "    const secs = total - minutes * 60;"
    yield "    const parts = [];"
    yield "    if (days) parts.push(days + 'd');"
    yield "    if (hours || parts.length) parts.push(hours + 'h');"
    yield "    if (minutes || parts.length) parts.push(minutes + 'm');"
    yield "    parts.push(secs + 's');"
    yield "    return parts.join(' ');"
    yield "  }"
    yield "  function numberFromDataset(panel, name){"
    yield "    const raw = Number(panel.dataset[name] || 0);"
    yield "    return Number.isFinite(raw) ? raw : 0;"
    yield "  }"
    yield "  function formatStatAgeFromEpoch(epoch, emptyText){"
    yield "    const raw = Number(epoch || 0);"
    yield "    if (!Number.isFinite(raw) || raw <= 0) return emptyText;"
    yield "    return formatStatDuration((Date.now() / 1000) - raw) + ' ago';"
    yield "  }"
    yield "  function statStorageKey(panel){"
    yield "    const kind = String(panel.dataset.statPanel || 'device');"
    yield "    const id = String(panel.dataset.statId || '').trim() || 'unknown';"
    yield "    return 'sensorius.stats.' + kind + '.' + id;"
    yield "  }"
    yield "  function readStatReset(panel){"
    yield "    try {"
    yield "      const raw = window.localStorage ? window.localStorage.getItem(statStorageKey(panel)) : '';"
    yield "      const parsed = raw ? JSON.parse(raw) : {};"
    yield "      return parsed && typeof parsed === 'object' ? parsed : {};"
    yield "    } catch (_err) {"
    yield "      return {};"
    yield "    }"
    yield "  }"
    yield "  function writeStatReset(panel, payload){"
    yield "    try {"
    yield "      if (window.localStorage) window.localStorage.setItem(statStorageKey(panel), JSON.stringify(payload));"
    yield "    } catch (_err) {}"
    yield "  }"
    yield "  function rebuildChannelRows(panel, rows){"
    yield "    const list = panel ? panel.querySelector('[data-stat-channel-list]') : null;"
    yield "    if (!list || !Array.isArray(rows)) return;"
    yield "    list.innerHTML = '';"
    yield "    rows.forEach(function(row){"
    yield "      const wrap = document.createElement('div');"
    yield "      wrap.className = 'statistics-row';"
    yield "      wrap.setAttribute('data-stat-channel-row', '');"
    yield "      wrap.dataset.channelId = String(row.channel_id || '');"
    yield "      wrap.dataset.state = String(row.state || '');"
    yield "      wrap.dataset.stateEpoch = row.state_epoch == null ? '' : String(row.state_epoch);"
    yield "      const label = document.createElement('span');"
    yield "      label.textContent = String(row.row_label || 'Switch current state, age:');"
    yield "      const value = document.createElement('strong');"
    yield "      value.setAttribute('data-stat-channel-value', '');"
    yield "      value.textContent = String(row.state_age_label || 'No state changes');"
    yield "      wrap.appendChild(label);"
    yield "      wrap.appendChild(value);"
    yield "      list.appendChild(wrap);"
    yield "    });"
    yield "  }"
    yield "  function renderStatsPanel(panel){"
    yield "    if (!panel) return;"
    yield "    const reset = readStatReset(panel);"
    yield "    const rawOffline = numberFromDataset(panel, 'offlineEvents24h');"
    yield "    const resetEpoch = Number(reset.resetEpoch || 0);"
    yield "    const offlineBase = resetEpoch > 0 ? Number(reset.offlineEventsBase || 0) : 0;"
    yield "    const offlineEl = panel.querySelector('[data-stat-value=\"offline-events\"]');"
    yield "    const uptimeEl = panel.querySelector('[data-stat-value=\"uptime\"]');"
    yield "    const lastPacketEl = panel.querySelector('[data-stat-value=\"last-packet\"]');"
    yield "    if (offlineEl) offlineEl.textContent = String(Math.max(0, rawOffline - offlineBase));"
    yield "    if (uptimeEl) {"
    yield "      const lastOfflineEpoch = Number(panel.dataset.lastOfflineEpoch || 0);"
    yield "      const baseEpoch = Math.max(lastOfflineEpoch > 0 ? lastOfflineEpoch : 0, resetEpoch > 0 ? resetEpoch : 0);"
    yield "      uptimeEl.textContent = baseEpoch > 0 ? formatStatDuration((Date.now() / 1000) - baseEpoch) : 'No offline events';"
    yield "    }"
    yield "    if (lastPacketEl) {"
    yield "      lastPacketEl.textContent = formatStatAgeFromEpoch(panel.dataset.lastPacketEpoch, 'No packets');"
    yield "    }"
    yield "    panel.querySelectorAll('[data-stat-channel-row]').forEach(function(row){"
    yield "      const value = row.querySelector('[data-stat-channel-value]');"
    yield "      const stateEpoch = Number(row.dataset.stateEpoch || 0);"
    yield "      const state = String(row.dataset.state || '').trim();"
    yield "      if (value) value.textContent = stateEpoch > 0 ? ((state || 'Unknown') + ', ' + formatStatDuration((Date.now() / 1000) - stateEpoch)) : 'No state changes';"
    yield "    });"
    yield "  }"
    yield "  function applyStatsPayload(panel, data){"
    yield "    if (!panel || !data || data.ok === false) return;"
    yield "    if (Object.prototype.hasOwnProperty.call(data, 'offline_events_24h')) panel.dataset.offlineEvents24h = String(data.offline_events_24h || 0);"
    yield "    if (Object.prototype.hasOwnProperty.call(data, 'last_offline_epoch')) panel.dataset.lastOfflineEpoch = data.last_offline_epoch == null ? '' : String(data.last_offline_epoch);"
    yield "    if (Object.prototype.hasOwnProperty.call(data, 'switch_last_packet_epoch')) panel.dataset.lastPacketEpoch = data.switch_last_packet_epoch == null ? '' : String(data.switch_last_packet_epoch);"
    yield "    const lastOfflineEl = panel.querySelector('[data-stat-value=\"last-offline\"]');"
    yield "    if (lastOfflineEl && data.last_offline_event_label) lastOfflineEl.textContent = String(data.last_offline_event_label);"
    yield "    rebuildChannelRows(panel, data.switch_channel_state_stats);"
    yield "    renderStatsPanel(panel);"
    yield "  }"
    yield "  async function refreshStatsPanel(panel){"
    yield "    if (!panel || !panel.dataset.statsUrl) return;"
    yield "    try {"
    yield "      const resp = await fetch(panel.dataset.statsUrl, {"
    yield "        cache: 'no-store',"
    yield "        headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' },"
    yield "      });"
    yield "      const data = await resp.json().catch(function(){ return {}; });"
    yield "      if (!resp.ok || data.ok === false) return;"
    yield "      applyStatsPayload(panel, data);"
    yield "    } catch (_err) {}"
    yield "  }"
    yield "  function isStatsPanelVisible(panel){"
    yield "    return !!panel && window.getComputedStyle(panel).display !== 'none' && !panel.hidden;"
    yield "  }"
    yield "  function initStatsPanels(){"
    yield "    const panels = modal.querySelectorAll('[data-stat-panel]');"
    yield "    panels.forEach(function(panel){ renderStatsPanel(panel); });"
    yield "    if (!modal.dataset.statsResetBound) {"
    yield "      modal.dataset.statsResetBound = '1';"
    yield "      modal.querySelectorAll('[data-stat-reset]').forEach(function(btn){"
    yield "        btn.addEventListener('click', function(){"
    yield "          const panel = btn.closest('[data-stat-panel]');"
    yield "          if (!panel) return;"
    yield "          writeStatReset(panel, {"
    yield "            resetEpoch: Date.now() / 1000,"
    yield "            offlineEventsBase: numberFromDataset(panel, 'offlineEvents24h')"
    yield "          });"
    yield "          renderStatsPanel(panel);"
    yield "        });"
    yield "      });"
    yield "    }"
    yield "    if (!modal.__statsTimer) {"
    yield "      modal.__statsTimer = window.setInterval(function(){"
    yield "        if (!document.body.contains(modal)) {"
    yield "          window.clearInterval(modal.__statsTimer);"
    yield "          modal.__statsTimer = null;"
    yield "          return;"
    yield "        }"
    yield "        panels.forEach(function(panel){ renderStatsPanel(panel); });"
    yield "      }, 1000);"
    yield "    }"
    yield "    if (!modal.__statsRefreshTimer) {"
    yield "      modal.__statsRefreshTimer = window.setInterval(function(){"
    yield "        if (!document.body.contains(modal)) {"
    yield "          window.clearInterval(modal.__statsRefreshTimer);"
    yield "          modal.__statsRefreshTimer = null;"
    yield "          return;"
    yield "        }"
    yield "        panels.forEach(function(panel){ if (isStatsPanelVisible(panel)) refreshStatsPanel(panel); });"
    yield "      }, 5000);"
    yield "    }"
    yield "  }"
    yield "  initStatsPanels();"
    yield ""
    yield "  function activate(which){"
    yield "    const showSettings = (which === 'settings');"
    yield "    const showStats = (which === 'statistics');"
    yield "    paneSettings.hidden = !showSettings;"
    yield "    paneStats.hidden = !showStats;"
    yield "    btnSettings.classList.toggle('active', showSettings);"
    yield "    btnStats.classList.toggle('active', showStats);"
    yield "    btnSettings.setAttribute('aria-selected', showSettings ? 'true' : 'false');"
    yield "    btnStats.setAttribute('aria-selected', showStats ? 'true' : 'false');"
    yield "  }"
    yield ""
    ...
    yield "  });"
    yield ""
    yield "  btnStats.onclick = function(){"
    yield "    activate('statistics');"
    yield "    initStatsPanels();"
    yield "    refreshStatsPanel(paneStats);"
    yield "  };"
    yield ""
    ...
    yield "        const savedLocation = locationInput ? String(locationInput.value || '').trim() : '';"
    yield "        if (savedLocation !== initialLocation) setTimeout(() => window.location.reload(), 400);"
    yield "      } catch (err) {"
    yield "        const msg = err && err.message ? err.message : 'Failed to save switch settings.';"
    yield "        setStatus(msg);"
    yield "        if (typeof window.showToast === 'function') window.showToast(msg, 'error');"
    yield "      } finally {"
    yield "        setBusy(false);"
    yield "      }"
    yield "    });"
    ...
    yield "        if (!switchId) {"
    yield "          const msg = 'Missing switch id.';"
    yield "          setStatus(msg);"
    yield "          if (typeof window.showToast === 'function') window.showToast(msg, 'error');"
    yield "          return;"
    yield "        }"
    ...
    yield "        } else if (msg.type === 'email_failure'){"
    yield "          const attempts = Math.max(1, Number(msg.attempts || 1));"
    yield "          const recipient = String(msg.recipient || '').trim();"
    yield "          const subject = String(msg.subject || 'Email notification').trim();"
    yield "          const error = String(msg.error || 'Unknown SMTP error').trim();"
    yield "          const text = `Email delivery failed after ${attempts} attempt${attempts === 1 ? '' : 's'}${recipient ? ` — To: ${recipient}` : ''} — ${subject} — ${error}`;"
    ...
    yield "            t.className = 'toast error email-failure-toast';"
    yield "            t.textContent = text;"
    yield "            t.title = 'Click to dismiss';"
    yield "            t.setAttribute('role', 'alert');"
    ...
    yield "          const conditions = Array.isArray(msg.trigger_conditions) ? msg.trigger_conditions.filter(Boolean).map(String) : [];"
    yield "          const triggerValues = Array.isArray(msg.trigger_values) ? msg.trigger_values.filter(value => value !== null && value !== undefined).map(String) : [];"
    ...
    yield "          const conditionText = conditions.length ? conditions.join(' OR ') : `${device} ${metric}`;"
    yield "          const valueText = triggerValues.length ? triggerValues.join('; ') : `${value}${unit}`;"
    yield "          const text = `${name} @ ${when}\n${conditionText}\n${valueText}`;"
    yield "          const c = window.getToastContainer ? window.getToastContainer(document.body) : null;"
    yield "          if (c) {"
    yield "            const t = document.createElement('div');"
    ...
    yield "          const when = parsedAt && !Number.isNaN(parsedAt.getTime())"
    ...
    yield "              : { background: String((msg.to && msg.to.color) || ''), text: '#27313a' };"
    yield "            if (/^#[0-9a-f]{6}$/i.test(actionColors.background)) {"
    yield "              t.style.backgroundColor = actionColors.background;"
    yield "              t.style.color = actionColors.text;"
    yield "            }"
    yield "            t.title = 'Click to dismiss';"
    yield "            t.setAttribute('role', 'status');"
    yield "            t.addEventListener('click', () => { t.remove(); if (!c.children.length) c.remove(); });"
    yield "            c.appendChild(t);"
    yield "          }"
    yield "        } else if (msg.type === 'switch_inventory_changed' || msg.type === 'dashboard_inventory_changed'){"
    yield "          const now = Date.now();"
    yield "          if ((now - __switchInventoryRefreshAt) > 1500) {"
    yield "            __switchInventoryRefreshAt = now;"
    yield "            if (typeof updateGauges === 'function') {"
    yield "              setTimeout(() => updateGauges({ ignoreVisibility: true, ignoreModal: true }), 120);"
    yield "            }"
    yield "          }"
    yield "        }"
    ...
    yield "function applySensorStatuses(data){"
    ...
    yield "  });"
    yield "}"
    yield "function isDashboardWarmingPayload(payload){"
    yield "  const reason = String((payload && payload.reason) || '').trim();"
    yield "  const cacheStatus = String((payload && payload.cache_status) || '').trim();"
    yield "  return reason === 'warming' || cacheStatus === 'warming';"
    yield "}"
    yield "function setDashboardCardLoading(cardId, isLoading){"
    yield "  const card = document.getElementById(cardId);"
    yield "  if (!card) return;"
    yield "  card.classList.toggle('card-loading', !!isLoading);"
    yield "  card.setAttribute('aria-busy', isLoading ? 'true' : 'false');"
    yield "}"
    yield "function setAstroCardsLoading(isLoading){"
    yield "  ['moonBox','sunBox','sunMoon29Card'].forEach((id) => setDashboardCardLoading(id, isLoading));"
    yield "}"
    yield "function setBiodynamicCardLoading(isLoading){"
    yield "  setDashboardCardLoading('bioBox', isLoading);"
    yield "}"
    yield "function applyAstroPayloadIfStable(payload){"
    yield "  if (!payload || typeof payload !== 'object') return;"
    yield "  const warming = isDashboardWarmingPayload(payload);"
    yield "  const keepExistingAstro = warming && astroData && astroData.ok;"
    yield "  setAstroCardsLoading(warming && !keepExistingAstro);"
    yield "  if (keepExistingAstro) return;"
    yield "  if (!payload.ok && astroData && astroData.ok && !payload.reason && !payload.cache_status) return;"
    yield "  if (!warming) { delete astroData.reason; delete astroData.cache_status; }"
    yield "  Object.assign(astroData, payload);"
    yield "  if (typeof drawSunPath === 'function') drawSunPath(astroData);"
    yield "  if (typeof drawMoonPhase === 'function') drawMoonPhase(astroData);"
    yield "  if (typeof drawSunMoon29Day === 'function' && sunMoon29IsOpen()) drawSunMoon29Day(astroData);"
    yield "}"
    yield "function applyBiodynamicPayloadIfStable(payload){"
    yield "  if (!payload || typeof payload !== 'object') return;"
    yield "  const warming = isDashboardWarmingPayload(payload);"
    yield "  const keepExistingBiodynamic = warming && biodynamicData && biodynamicData.ok;"
    yield "  setBiodynamicCardLoading(warming && !keepExistingBiodynamic);"
    yield "  if (keepExistingBiodynamic) return;"
    yield "  Object.keys(biodynamicData).forEach((k) => { delete biodynamicData[k]; });"
    yield "  Object.assign(biodynamicData, payload);"
    yield "  if (typeof drawBiodynamic === 'function') drawBiodynamic(biodynamicData);"
    yield "}"
    ...
    yield "    const now = Date.now();"
    yield "    let data = null;"
    yield "    const extrasWarming = !!window.__dashboardExtrasWarming;"
    yield "    const lastExtrasRefreshAt = window.__lastExtrasRefreshAtMs || 0;"
    yield "    const lastExtrasWarmAt = window.__lastExtrasWarmAtMs || 0;"
    yield "    const wantExtras = !lastExtrasRefreshAt || ((now - lastExtrasRefreshAt) >= dashboardExtrasRefreshMs) || (extrasWarming && (!lastExtrasWarmAt || ((now - lastExtrasWarmAt) >= dashboardExtrasWarmRetryMs)));"
    ...
    yield "      if (wantExtras) {"
    yield "        const astroWarming = data && data.astro && isDashboardWarmingPayload(data.astro);"
    yield "        const biodynamicWarming = data && data.biodynamic && isDashboardWarmingPayload(data.biodynamic);"
    yield "        if (astroWarming || biodynamicWarming) {"
    yield "          window.__dashboardExtrasWarming = true;"
    yield "          window.__lastExtrasWarmAtMs = now;"
    yield "        } else {"
    yield "          window.__dashboardExtrasWarming = false;"
    yield "          window.__lastExtrasRefreshAtMs = now;"
    yield "          window.__lastExtrasWarmAtMs = 0;"
    yield "        }"
    yield "      }"
    yield "    }"
    yield "    applySensorStatuses(data);"
    yield "    if (data && data.astro && typeof data.astro === 'object') {"
    yield "      applyAstroPayloadIfStable(data.astro);"
    yield "    }"
    yield "    if (data && data.biodynamic && typeof data.biodynamic === 'object') {"
    yield "      applyBiodynamicPayloadIfStable(data.biodynamic);"
    yield "    }"
    ...
    yield "  setInterval(() => updateGauges({ ignoreModal: true }), 15000);"
    ...
    yield "      window.refreshAllMicrographs(false, { ignoreModal: true });"
    yield "    }"
    yield "  }, 60000);"
    yield "}"
    ...
    yield "<div class='dashboard-overview-graphic' id='dashboard-overview-footer'>"
...
def core_helpers_html() -> str:
    ...
    return (
        "<style>"
        ".toast-container{position:fixed; top:16px; left:50%; width:min(92vw,520px); transform:translateX(-50%); z-index:99999; display:flex; flex-direction:column; align-items:center; gap:8px; pointer-events:none}"
        ".toast-container.modal-pane-toast-container{position:absolute; top:12px; width:min(32rem,calc(100% - 1.5rem)); z-index:100000}"
        ".system-settings-content,.switch-dialog-content,.modal .split > section.panel{position:relative}"
        ...
        "<script>"
        "(function(){"
        "  if (typeof window.getToastContainer !== 'function') {"
        "    window.getToastContainer = function(context){"
        "      function visible(el){"
        ...
        "      }"
        "      var paneSelectors = ['.system-settings-content', '.switch-dialog-content', '.modal-body .split > section.panel'];"
        "      function paneFrom(root){"
        "        if (!root || !document.body.contains(root)) return null;"
        "        var selectors = paneSelectors;"
        "        for (var i = 0; i < selectors.length; i += 1) {"
        "          if (root.matches && root.matches(selectors[i]) && visible(root)) return root;"
        "          var pane = root.querySelector(selectors[i]);"
        "          if (visible(pane)) return pane;"
        "        }"
        "        return null;"
        "      }"
        "      function contextRoot(){"
        "        if (!context) return null;"
        "        var el = context.nodeType === 1 ? context : context.parentElement;"
        "        if (!el || !document.body.contains(el)) return null;"
        "        return el.closest('.modal-backdrop') || el.closest('.system-settings-shell, .modal') || el;"
        "      }"
        "      function activePane(){"
        "        var contextual = paneFrom(contextRoot());"
        "        if (contextual) return contextual;"
        "        var roots = Array.prototype.filter.call(document.querySelectorAll('.modal-backdrop, .system-settings-shell, .modal'), visible);"
        "        for (var i = roots.length - 1; i >= 0; i -= 1) {"
        "          var pane = paneFrom(roots[i]);"
        "          if (pane) return pane;"
        "        }"
        "        var panes = Array.prototype.filter.call(document.querySelectorAll(paneSelectors.join(',')), visible);"
        ...
        "        var c = children.find(function(el){ return el.classList && el.classList.contains('toast-container') && el.classList.contains('modal-pane-toast-container'); });"
        "        if (!c) { c = document.createElement('div'); c.className = 'toast-container modal-pane-toast-container'; pane.appendChild(c); }"
        "        return c;"
        "      }"
        "      var bodyToast = document.querySelector('body > .toast-container:not(.modal-pane-toast-container)');"
        "      if (!bodyToast) { bodyToast = document.createElement('div'); bodyToast.className = 'toast-container'; document.body.appendChild(bodyToast); }"
        "      return bodyToast;"
        "    };"
        "  }"
        "  if (typeof window.showToast !== 'function') {"
        "    window.showToast = function(text, type, context){"
        "      var c = window.getToastContainer ? window.getToastContainer(context) : null;"
        "      if (!c) return;"
