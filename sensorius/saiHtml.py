...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, expected_display_style_map=None, display_style=None, astro_payload=None, biodynamic_payload=None, weather_forecast_provider="met_no", weather_forecast_theme="garden", dashboard_background_theme="leaf", dashboard_metric_set="Pick 6"):
    ...
    yield "    let speed = speedByTimestamp.get(String(directionTs[index]));"
    yield "    if (!Number.isFinite(speed) && directions.length === speeds.length) speed = Number(speeds[index]);"
    ...
    yield "  });"
    yield "  ctx.textAlign = 'right';"
    yield "  ctx.fillText('mph', width - 3, legendY);"
    yield "  ctx.restore();"
    yield "  const averageSpeed = speedTotal / sampleCount;"
    yield "  canvas.setAttribute('role', 'img');"
    yield "  canvas.setAttribute('aria-label', `${rangeLabel} wind rose. Dominant direction ${directionNames[dominantIndex]} at ${dominantPercent} percent. Average wind speed ${averageSpeed.toFixed(1)} mph.`);"
    yield "  canvas.title = `${rangeLabel} wind rose — ${sampleCount} samples — dominant ${directionNames[dominantIndex]} ${dominantPercent}% — average ${averageSpeed.toFixed(1)} mph`;"
    yield "  return true;"
    yield "}"
