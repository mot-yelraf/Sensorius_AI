...
def render_dashboard(sensor_id, sensor, available, all_values, all_stats, mqtt_ingest, switch_controllers=None, sensor_locations=None, gauge_config=None, gauge_size="Small", expected_gauge_map=None, expected_display_style_map=None, display_style=None, astro_payload=None, biodynamic_payload=None, weather_forecast_provider="met_no", weather_forecast_theme="garden"):
    ...
    yield "  const mrToday = toMin(data && data.moon_rise_today);"
    yield "  const msToday = toMin(data && data.moon_set_today);"
    ...
    yield "    const riseMin = mrToday;"
    yield "    const setMin = msToday;"
    yield "    if (!Number.isFinite(riseMin) || !Number.isFinite(setMin) || riseMin >= setMin) return [];"
    yield "    const visible = moonPoints.filter((p) => p.m >= riseMin && p.m <= setMin && Number.isFinite(p.e));"
    yield "    let peak = null;"
    yield "    for (const p of visible) {"
    yield "      if (!peak || p.e > peak.e) peak = p;"
    yield "    }"
    yield "    const peakMin = peak ? Math.max(riseMin + 1, Math.min(setMin - 1, peak.m)) : ((riseMin + setMin) / 2);"
    yield "    const peakYRaw = peak ? yForElev(peak.e) : (yBase - (abovePx * 0.95));"
    yield "    const topY = Math.max(padY + 2, Math.min(yBase - 6, peakYRaw));"
    yield "    const bottomY = yBase + belowPx;"
    yield "    const prevMoonSet = setMin - 1440;"
    yield "    const nextMoonRise = riseMin + 1440;"
    yield "    const prevTrough = prevMoonSet + ((riseMin - prevMoonSet) / 2);"
    yield "    const nextTrough = setMin + ((nextMoonRise - setMin) / 2);"
    yield "    const riseSlope = -Math.min(((yBase - topY) * Math.PI) / (2 * Math.max(1, peakMin - riseMin)), ((bottomY - yBase) * Math.PI) / (2 * Math.max(1, riseMin - prevTrough)));"
    yield "    const setSlope = Math.min(((yBase - topY) * Math.PI) / (2 * Math.max(1, setMin - peakMin)), ((bottomY - yBase) * Math.PI) / (2 * Math.max(1, nextTrough - setMin)));"
    yield "    const keys = ["
    yield "      {m: prevMoonSet, y: yBase, s: setSlope},"
    yield "      {m: prevTrough, y: bottomY, s: 0},"
    yield "      {m: riseMin, y: yBase, s: riseSlope},"
    yield "      {m: peakMin, y: topY, s: 0},"
    yield "      {m: setMin, y: yBase, s: setSlope},"
    yield "      {m: nextTrough, y: bottomY, s: 0},"
    yield "      {m: nextMoonRise, y: yBase, s: riseSlope},"
    yield "    ];"
    yield "    const yAt = (minute) => {"
    yield "      if (minute <= keys[0].m) return keys[0].y;"
    ...
    yield "        }"
    yield "      }"
    yield "      return keys[keys.length - 1].y;"
    yield "    };"
    yield "    const built = [];"
    yield "    for (let m = 0; m <= 1440; m += 5) {"
    ...
    yield "    const sampled = softenMoonLowerExtrema(smoothSampledElevationPath(moonPoints, 2));"
