...
Native packages generated by `platform_installers/installer_generator.py`
also write a credential-safe, append-only installation summary. Linux and
Raspberry Pi packages use `/var/log/sensorius/install.log`, macOS packages use
`/Library/Logs/Sensorius/install.log`, and Windows packages use
`%LOCALAPPDATA%\Sensorius\Logs\install.log`. More detailed native installer
history remains available in `/var/log/dpkg.log` on Debian-family systems,
`/var/log/install.log` on macOS, and a timestamped Inno Setup log under
`%TEMP%` on Windows. These Sensorius summaries do not include `.env` contents,
credentials, API keys, MQTT payloads, or Wi-Fi passwords.
